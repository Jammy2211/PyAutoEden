...
class SphPowerLawCored(EllPowerLawCored):
    def __init__(
        self, centre=(0.0, 0.0), einstein_radius=1.0, slope=2.0, core_radius=0.01
    ):
        """
...
class EllPowerLaw(EllPowerLawCored):
    ...
    def deflections_yx_2d_from(self, grid):
        ...
        angle = np.arctan2(grid[:, 0], np.multiply(self.axis_ratio, grid[:, 1]))
        R = np.sqrt(
            np.add(
                np.multiply((self.axis_ratio ** 2), (grid[:, 1] ** 2)),
                (grid[:, 0] ** 2),
            )
        )
        z = np.add(
        ...
        complex_angle = (
            (((2.0 * b) / (1.0 + self.axis_ratio)) * ((b / R) ** (slope - 1.0))) * z
        ) * special.hyp2f1(
            1.0, (0.5 * slope), (2.0 - (0.5 * slope)), ((-factor) * (z ** 2))
        )
...
class EllIsothermal(EllPowerLaw):
    def __init__(
        self, centre=(0.0, 0.0), elliptical_comps=(0.0, 0.0), einstein_radius=1.0
    ):
        """
    ...
    def deflections_yx_2d_from(self, grid):
        ...
        deflection_y = np.arctanh(
            np.divide(
                np.multiply(np.sqrt((1 - (self.axis_ratio ** 2))), grid[:, 0]), psi
            )
        )
        deflection_x = np.arctan(
            np.divide(
                np.multiply(np.sqrt((1 - (self.axis_ratio ** 2))), grid[:, 1]), psi
            )
        )
        return self.rotate_grid_from_reference_frame(
