...
class ExternalShear(MassProfile):
    def __init__(self, elliptical_comps=(0.0, 0.0)):
        """
    ...
    def deflections_yx_2d_from(self, grid):
        if (self.preload_grid is not None) and (self.preload_deflections is not None):
            if grid.sub_shape_slim == self.preload_grid.sub_shape_slim:
                if np.allclose(grid, self.preload_grid, 1e-08):
                    return self.normalization_scale * self.preload_deflections
        if (self.preload_blurring_grid is not None) and (
            self.preload_blurring_deflections is not None
        ):
            if grid.sub_shape_slim == self.preload_blurring_grid.sub_shape_slim:
                if np.allclose(grid, self.preload_blurring_grid, 1e-08):
        ...
        if np.isnan(deflections_y).any() or np.isnan(deflections_x).any():
            raise exc.ProfileException(
                "The grid input into the DefectionsInput.deflections_yx_2d_from() method has (y,x)coodinates extending beyond the input image_plane_grid.Update the image_plane_grid to include deflection angles reaching to largerradii or reduce the input grid. "
            )
