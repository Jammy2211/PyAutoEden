...
import SHE_SLModel_Autoarray as aa
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.geometry_profiles import EllProfile
...
class MassProfile(EllProfile, OperateDeflections):
    ...
    def deflections_2d_via_potential_2d_from(self, grid):
        ...
        return aa.Grid2D.manual_mask(
            grid=np.stack((deflections_y_2d, deflections_x_2d), axis=(-1)),
            mask=grid.mask,
        )
    ...
    def convergence_1d_from(self, grid, radial_grid_shape_slim=None):
    ...
    @aa.grid_dec.grid_1d_to_structure
    def potential_1d_from(self, grid, radial_grid_shape_slim=None):
    ...
    def mass_angular_within_circle_from(self, radius):
        """
    ...
    def density_between_circular_annuli(self, inner_annuli_radius, outer_annuli_radius):
        """Calculate the mass between two circular annuli and compute the density by dividing by the annuli surface
        area.

        The value returned by the mass integral is dimensionless, therefore the density between annuli is returned in         unit_label of inverse radius squared. A conversion factor can be specified to convert this to a physical value         (e.g. the critical surface mass density).
    ...
    def average_convergence_of_1_radius(self):
        ...
        def func(radius):
            return self.mass_angular_within_circle_from(radius=radius) - (
                np.pi * (radius ** 2.0)
            )

        return (
            self.ellipticity_rescale * root_scalar(func, bracket=[0.0001, 10000.0]).root
        )
    ...
    def normalization_via_mass_angular_from(
        ...
        normalization_max=1000000000000000.0,
        bins=200,
    ):
        ...
        if (
            ...
            raise exc.ProfileException(
                "The normalization could not be computed from the Einstein Radius via the average of the convergence. The input einstein_radius may be too small or large to feasibly be computed by integrating the convergence. Alternative the normalization range or number of bins may need to be changed to capture the true einstein_radius value."
            )
    ...
    def normalization_via_einstein_radius_from(
        self,
        einstein_radius,
        normalization_min=1e-09,
        normalization_max=1000000000.0,
        bins=100,
    ):
        normalization_list = np.logspace(
            np.log10(normalization_min), np.log10(normalization_max), bins
...
class MassProfileMGE:
    ...
    def kesi(p):
        ...
        return (
            (((2.0 * p) * np.log(10)) / 3.0) + (((2.0 * np.pi) * n_list) * 1j)
        ) ** 0.5
    ...
    def eta(p):
        ...
        for i in np.arange(1, p, 1):
            kesi_list[((2 * p) - i)] = kesi_list[(((2 * p) - i) + 1)] + (
                (2 ** (-p)) * comb(p, i)
            )
        for i in np.arange(0, ((2 * p) + 1), 1):
            eta_list[i] = (
                ((((-1) ** i) * 2.0) * np.sqrt((2.0 * np.pi))) * (10 ** (p / 3.0))
            ) * kesi_list[i]
        return eta_list
    ...
    def _decompose_convergence_via_mge(
        ...
        -------
        """
        kesis = self.kesi(func_terms)
        etas = self.eta(func_terms)
        ...
        for i in range(func_gaussians):
            ...
            if (i == (-1)) or (i == (func_gaussians - 1)):
                amplitude_list[i] = ((0.5 * f_sigma) * d_log_sigma) / np.sqrt(
                    (2.0 * np.pi)
                )
            else:
...
def w_f_approx(z):
    ...
    reg5 = (((62.0 > abs_z2) & np.logical_not(reg4)) & (abs_z2 > 2.5)) & (
        z_imag2 < 0.072
    )
    if np.any(reg5):
        t = z[reg5]
    ...
    if np.any(reg6):
        ...
        s2 = [10.479857, 53.992907, 170.354, 348.70392, 457.33448, 352.73063, 122.60793]
        for s in s1:
            f1 = (f1 * t3) + s
        for s in s2:
            f2 = (f2 * t3) + s
...
class MassProfileCSE(ABC):
    @staticmethod
    def convergence_cse_1d_from(grid_radii, core_radius):
        """
    ...
    def deflections_via_cse_from(
        term1, term2, term3, term4, axis_ratio_squared, core_radius
    ):
        """
    ...
    def _decompose_convergence_via_cse_from(
        self, func, radii_min, radii_max, total_cses=25, sample_points=100
    ):
        """
