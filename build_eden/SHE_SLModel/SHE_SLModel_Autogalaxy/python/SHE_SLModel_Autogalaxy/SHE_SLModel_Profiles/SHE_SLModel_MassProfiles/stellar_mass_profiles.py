...
pip install pyquad"""
        ...
        def calculate_deflection_component(npow, index):
            ...
            deflection_grid *= (self.intensity * self.mass_to_light_ratio) * quad_grid(
                ...
                * (
                    ...
                    - (
                        np.exp(
                            (
                                ((-(xs_1 ** 2.0)) * (1.0 - q2))
                                - ((ys_1 * ys_1) * ((1.0 / q2) - 1.0))
                            )
                        )
                        * wofz(
                            ((self.axis_ratio * xs_1) + ((1j * ys_1) / self.axis_ratio))
                        )
                    )
                )
            )
...
class AbstractEllSersic(MassProfile, MassProfileMGE, MassProfileCSE, StellarProfile):
    ...
    def elliptical_effective_radius(self):
        """
        The effective_radius of a Sersic light profile is defined as the circular effective radius. This is the         radius within which a circular aperture contains half the profiles's total integrated light. For elliptical         systems, this won't robustly capture the light profile's elliptical shape.

        The elliptical effective radius instead describes the major-axis radius of the ellipse containing         half the light, and may be more appropriate for highly flattened systems like disk galaxies.
        """
...
pip install pyquad"""
    ...
    def decompose_convergence_via_mge(self):
        ...
        def sersic_radial_gradient_2D(r):
            return (
                (self.mass_to_light_ratio * self.intensity)
                * (
...
class EllSersicCore(EllSersic):
    ...
    def decompose_convergence_via_mge(self):
        ...
        def core_sersic_2D(r):
            return (
                (self.mass_to_light_ratio * self.intensity_prime)
                * (
