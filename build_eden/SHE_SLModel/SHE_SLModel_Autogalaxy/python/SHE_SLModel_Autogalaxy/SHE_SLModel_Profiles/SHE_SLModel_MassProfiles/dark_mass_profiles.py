...
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.SHE_SLModel_MassProfiles.mass_profiles import (
    MassProfileMGE,
    MassProfileCSE,
)
from SHE_SLModel_Autogalaxy import exc
from SHE_SLModel_Autogalaxy.SHE_SLModel_Util import cosmology_util
...
class AbstractEllNFWGeneralized(MassProfile, DarkProfile, MassProfileMGE):
    ...
    @aa.grid_dec.transform
    @aa.grid_dec.relocate_to_radial_minimum
    def convergence_2d_via_mge_from(self, grid):
        """Calculate the projected convergence at a given set of arc-second gridded coordinates.
    ...
    def tabulate_integral(self, grid, tabulate_bins):
        """Tabulate an integral over the convergence of deflection potential of a mass profile. This is used in         the GeneralizedNFW profile classes to speed up the integration procedure.
    ...
    def decompose_convergence_via_mge(self):
        rho_at_scale_radius = self.kappa_s / self.scale_radius
        radii_min = self.scale_radius / 2000.0
        radii_max = self.scale_radius * 30.0

        def gnfw_3d(r):
            x = r / self.scale_radius
            return (rho_at_scale_radius * (x ** (-self.inner_slope))) * (
    ...
    def delta_concentration(
        ...
        if redshift_of_cosmic_average_density == "profile":
            ...
            raise exc.UnitsException(
                "The redshift of the cosmic average density haas been specified as an invalid string. Must be {local, profile}"
            )
...
class EllNFWGeneralized(AbstractEllNFWGeneralized):
    ...
    def deflections_2d_via_integral_from(self, grid, tabulate_bins=1000):
        ...
        def calculate_deflection_component(npow, yx_index):
            ...
            for i in range(grid.shape[0]):
                deflection_grid[i] = (
                    (((2.0 * self.kappa_s) * self.axis_ratio) * grid[(i, yx_index)])
                    * quad(
    ...
    def convergence_func(self, grid_radius):
        ...
        for index in range(grid_radius.shape[0]):
            ...
            grid_radius[index] = (
                (2.0 * self.kappa_s) * (grid_radius[index] ** (1 - self.inner_slope))
            ) * (
    ...
    def potential_func(
        ...
        angle = potential_integral[i] + (
            ((potential_integral[(i + 1)] - potential_integral[i]) * (eta_u - r1))
            / (r2 - r1)
        )
...
class SphNFWGeneralized(EllNFWGeneralized):
    def __init__(
        self, centre=(0.0, 0.0), kappa_s=0.05, inner_slope=1.0, scale_radius=1.0
    ):
        """
...
class SphNFWTruncated(AbstractEllNFWGeneralized):
    def __init__(
        self, centre=(0.0, 0.0), kappa_s=0.05, scale_radius=1.0, truncation_radius=2.0
    ):
        super().__init__(
            centre=centre,
            elliptical_comps=(0.0, 0.0),
    ...
    def mass_at_truncation_radius_solar_mass(
        ...
        return (
            mass_at_200 * ((self.tau ** 2.0) / (((self.tau ** 2.0) + 1.0) ** 2.0))
        ) * (
...
class EllNFW(EllNFWGeneralized, MassProfileCSE):
    ...
    @aa.grid_dec.transform
    @aa.grid_dec.relocate_to_radial_minimum
    def deflections_2d_via_integral_from(self, grid):
        """
        Calculate the deflection angles at a given set of arc-second gridded coordinates.
...
pip install pyquad"""
    ...
    @aa.grid_dec.transform
    @aa.grid_dec.relocate_to_radial_minimum
    def deflections_2d_via_cse_from(self, grid):
    ...
    def deflection_func(u, y, x, npow, axis_ratio, scale_radius):
        ...
        if eta_u > 1:
            ...
            eta_u_2 = 1
        return ((2.0 * (1 - eta_u_2)) / ((eta_u ** 2) - 1)) / (
    ...
    @aa.grid_dec.transform
    @aa.grid_dec.relocate_to_radial_minimum
    def convergence_2d_via_cse_from(self, grid):
        """
    ...
    def potential_func(u, y, x, axis_ratio, kappa_s, scale_radius):
        ...
        if eta_u > 1:
            ...
            eta_u_2 = 1
        return (
            ((((4.0 * kappa_s) * scale_radius) * (axis_ratio / 2.0)) * (eta_u / u))
