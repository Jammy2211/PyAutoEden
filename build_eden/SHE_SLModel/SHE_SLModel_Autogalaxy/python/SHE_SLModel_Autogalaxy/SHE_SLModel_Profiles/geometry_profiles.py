...
from SHE_SLModel_Autoarray.SHE_SLModel_Structures.SHE_SLModel_Grids.SHE_SLModel_TwoD.grid_transformed import (
...
class SphProfile(GeometryProfile):
    ...
    def transform_grid_to_reference_frame(self, grid):
        """
        Transform a grid of (y,x) coordinates to the reference frame of the profile, including a translation to         its centre.
    ...
    def transform_grid_from_reference_frame(self, grid):
        """
        Transform a grid of (y,x) coordinates from the reference frame of the profile to the original observer         reference frame, including a translation from the profile's centre.
...
class EllProfile(SphProfile):
    def __init__(self, centre=(0.0, 0.0), elliptical_comps=(0.0, 0.0)):
        """ An elliptical profile, which describes profiles with y and x centre Cartesian coordinates, an axis-ratio         and rotational angle.
    ...
    @classmethod
    def from_axis_ratio_and_phi(cls, centre=(0.0, 0.0), axis_ratio=1.0, angle=0.0):
