...
from SHE_SLModel_Autoarray.SHE_SLModel_Dataset.abstract_dataset import (
    AbstractSettingsDataset,
)
from SHE_SLModel_Autoarray.SHE_SLModel_Dataset.abstract_dataset import AbstractDataset
...
class DatasetQuantity(AbstractDataset):
    def __init__(self, data, noise_map, settings=SettingsQuantity()):
        """
    ...
    def via_signal_to_noise_map(
        cls, data, signal_to_noise_map, settings=SettingsQuantity()
    ):
        """
        Represents a derived quantity of a light profile, mass profile, galaxy or plane as a dataset that can be fitted
