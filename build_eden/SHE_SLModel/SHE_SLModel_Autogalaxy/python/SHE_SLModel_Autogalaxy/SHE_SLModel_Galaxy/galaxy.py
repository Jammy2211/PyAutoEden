...
from SHE_SLModel_Autoconf.dictable import Dictable
from SHE_SLModel_Autogalaxy import exc
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.geometry_profiles import (
    GeometryProfile,
)
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.SHE_SLModel_LightProfiles.light_profiles import (
    LightProfile,
)
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.SHE_SLModel_MassProfiles import (
    MassProfile,
)
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.point_sources import Point
...
{}""".format(
                """
""".join(
                    map(str, self.light_profile_list)
                )
            )
...
{}""".format(
                """
""".join(
                    map(str, self.mass_profile_list)
                )
            )
        return string
    ...
    def convergence_2d_from(self, grid):
        ...
        if self.has_mass_profile:
            return sum(
                map(
                    (lambda p: p.convergence_2d_from(grid=grid)), self.mass_profile_list
                )
            )
        return np.zeros((grid.shape[0],))
    ...
    def convergence_1d_from(self, grid):
        ...
        if self.has_mass_profile:
            return sum(
                map(
                    (lambda p: p.convergence_1d_from(grid=grid)), self.mass_profile_list
                )
            )
        return np.zeros((grid.shape[0],))
    ...
    def luminosity_within_circle_from(self, radius):
        """
    ...
    def mass_angular_within_circle_from(self, radius):
        ...
        critical_surface_density or None
            The critical surface mass density of the strong lens configuration, which converts mass from angulalr             unit_label to phsical unit_label (e.g. solar masses).
        """
        if self.has_mass_profile:
            return sum(
                map(
...
class HyperGalaxy:
    ...
    def __init__(self, contribution_factor=0.0, noise_factor=0.0, noise_power=1.0):
        """
        If a `Galaxy` is given a *HyperGalaxy* as an attribute, the noise-map in         the regions of the image that the galaxy is located will be hyper, to prevent         over-fitting of the galaxy.
        
        This is performed by first computing the hyper_galaxies-galaxy's 'contribution-map',         which determines the fraction of flux in every pixel of the image that can be         associated with this particular hyper_galaxies-galaxy. This is computed using         hyper_galaxies-hyper_galaxies set (e.g. fitting.fit_data.FitDataHyper), which includes  best-fit         unblurred_image_1d of the galaxy's light from a previous analysis search.
         
        The *HyperGalaxy* class contains the hyper_galaxies-parameters which are associated         with this galaxy for scaling the noise-map.
