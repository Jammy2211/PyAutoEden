...
from SHE_SLModel_Autogalaxy.SHE_SLModel_Plane.plane import PlaneImage
from SHE_SLModel_Autogalaxy import exc


def plane_image_of_galaxies_from(shape, grid, galaxies, buffer=0.01):
...
def ordered_plane_redshifts_with_slicing_from(
    ...
    """Given a set of lens plane redshifts, the source-plane redshift and the number of planes between each, setup the     plane redshifts using these values. A lens redshift corresponds to the 'main' lens galaxy(s),
    whereas the slices collect line-of-sight halos over a range of redshifts.

    The source-plane redshift is removed from the ordered plane redshifts that are returned, so that galaxies are not     planed at the source-plane redshift.

    For example, if the main plane redshifts are [1.0, 2.0], and the bin sizes are [1,3], the following redshift     slices for planes will be used:
    ...
    planes_between_lenses : [int]
        The number of slices between each main plane. The first entry in this list determines the number of slices         between Earth (redshift 0.0) and main plane 0, the next between main planes 0 and 1, etc.
    source_plane_redshift
        The redshift of the source-plane, which is input explicitly to ensure galaxies are not placed in the         source-plane.
    """
    if len(lens_redshifts) != (len(planes_between_lenses) - 1):
        raise exc.PlaneException(
            "The number of lens_plane_redshifts input is not equal to the number of slices_between_lens_planes+1."
        )
    plane_redshifts = []
...
def galaxies_in_redshift_ordered_planes_from(galaxies, plane_redshifts):
    """Given a list of galaxies (with redshifts), return a list of the galaxies where each entry contains a list     of galaxies at the same redshift in ascending redshift order.
