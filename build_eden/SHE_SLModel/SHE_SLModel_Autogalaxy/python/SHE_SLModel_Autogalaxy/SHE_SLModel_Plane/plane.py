...
from SHE_SLModel_Autoarray.SHE_SLModel_Inversion.SHE_SLModel_Inversion.factory import (
    inversion_imaging_unpacked_from,
)
from SHE_SLModel_Autoarray.SHE_SLModel_Inversion.SHE_SLModel_Inversion.factory import (
...
from SHE_SLModel_Autogalaxy.SHE_SLModel_Profiles.SHE_SLModel_LightProfiles.light_profiles_snr import (
    LightProfileSNR,
)
...
from SHE_SLModel_Autogalaxy.SHE_SLModel_Util import plane_util


class Plane(OperateImageGalaxies, OperateDeflections, Dictable):
    def __init__(self, galaxies, redshift=None, profiling_dict=None):
        ...
        if redshift is None:
            if not galaxies:
                raise exc.PlaneException(
                    "No redshift and no galaxies were input to a Plane. A redshift for the Plane therefore cannot bedetermined"
                )
            elif not all(
    ...
    def galaxy_image_2d_dict_from(self, grid):
        """
    ...
    @property
    def mass_profile_list_of_galaxies(self):
        return [
    ...
    def convergence_2d_from(self, grid):
        ...
        -----------
        grid : Grid2D
            The grid (or sub) of (y,x) arc-second coordinates at the centre of every unmasked pixel which the             potential is calculated on.
        galaxies : [Galaxy]
    ...
    @property
    def hyper_galaxies_with_pixelization_image_list(self):
    ...
    def set_snr_of_snr_light_profiles(
        self, grid, exposure_time, background_sky_level=0.0
    ):
        for galaxy in self.galaxies:
