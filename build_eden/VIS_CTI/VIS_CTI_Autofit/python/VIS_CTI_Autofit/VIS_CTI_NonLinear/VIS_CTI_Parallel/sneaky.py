...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Parallel.process import (
    AbstractJob,
    Process,
    StopCommand,
)
...
def _is_likelihood_function(function):
    """
...
class SneakyJob(AbstractJob):
    ...
    def perform(self, likelihood_function):
        ...
        args = (self.args[: self.fitness_index] + [likelihood_function]) + self.args[
            self.fitness_index :
        ]
...
class SneakyProcess(Process):
    def __init__(self, name, paths, initializer=None, initargs=None, job_args=tuple()):
        """
...
class SneakyPool:
    def __init__(self, processes, fitness, paths, initializer=None, initargs=None):
        """
    ...
    def map(self, function, args_list):
        ...
        jobs = [
            SneakyJob(
                function,
                *((args,) if (not isinstance(args, Iterable)) else tuple(args)),
            )
