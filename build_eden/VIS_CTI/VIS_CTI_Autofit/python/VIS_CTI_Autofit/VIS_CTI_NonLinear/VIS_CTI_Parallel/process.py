...
class Process(multiprocessing.Process):
    def __init__(
        self, name, job_queue, initializer=None, initargs=None, job_args=tuple()
    ):
        """
    ...
    def _init(self):
        if self.initializer is not None:
            ...
            if isinstance(self.initargs, collections.Iterable):
                initargs = tuple(self.initargs)
            else:
                initargs = (self.initargs,)
            self.initializer(*initargs)
    ...
    @classmethod
    def run_jobs(
        cls, jobs, number_of_cores, initializer=None, initargs=None, job_args=tuple()
    ):
        """
        ...
        for job in jobs:
            job_queue.put(job)
            total += 1
        logger.info(f"Running {total} jobs across {number_of_cores} processes")
