...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Nest.VIS_CTI_Ultranest.samples import (
    UltraNestSamples,
)
from VIS_CTI_Autofit.VIS_CTI_Plot import UltraNestPlotter
...
class UltraNest(abstract_nest.AbstractNest):
    ...
    def config_dict_stepsampler(self):
        config_dict = copy.copy(
            self.config_type[self.__class__.__name__]["stepsampler"]._dict
        )
    ...
    def _fit(self, model, analysis, log_likelihood_cap=None):
        ...
        while not finished:
            ...
            if (total_iterations == iterations_after_run) or (
                iterations_after_run == self.config_dict_run["max_ncalls"]
            ):
                finished = True
