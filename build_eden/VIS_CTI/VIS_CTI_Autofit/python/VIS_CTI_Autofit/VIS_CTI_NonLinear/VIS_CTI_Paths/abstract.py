...
from VIS_CTI_Autofit.VIS_CTI_Mapper.identifier import Identifier, IdentifierField
from VIS_CTI_Autofit.VIS_CTI_Text import text_util
from VIS_CTI_Autofit.VIS_CTI_Tools.util import open_, zip_directory
...
class AbstractPaths(ABC):
    ...
    def save_summary(self, samples, log_likelihood_function_time):
        text_util.results_to_file(
            samples=samples, filename=path.join(self.output_path, "model.results")
        )
