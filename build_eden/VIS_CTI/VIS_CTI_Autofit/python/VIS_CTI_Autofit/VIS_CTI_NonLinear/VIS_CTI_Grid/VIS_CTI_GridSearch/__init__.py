...
from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_Prior import prior as p
from VIS_CTI_Autofit.VIS_CTI_NonLinear.abstract_search import NonLinearSearch
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Parallel import Process
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Grid.VIS_CTI_GridSearch.job import Job
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Grid.VIS_CTI_GridSearch.result import (
    GridSearchResult,
)
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Grid.VIS_CTI_GridSearch.result_builder import (
    ResultBuilder,
)
...
class GridSearch:
    def __init__(
        self, search, number_of_steps=4, number_of_cores=1, result_output_interval=100
    ):
        """
    ...
    def make_physical_lists(self, grid_priors):
    ...
    def make_arguments(self, values, grid_priors):
        ...
        for (value, grid_prior) in zip(values, grid_priors):
            if (float("-inf") == grid_prior.lower_limit) or (
                float("inf") == grid_prior.upper_limit
            ):
    ...
    def make_jobs(self, model, analysis, grid_priors, info=None):
        grid_priors = list(set(grid_priors))
        lists = self.make_lists(grid_priors)
        jobs = list()
...
""".join(
    ...
    def job_for_analysis_grid_priors_and_values(
        self, model, analysis, grid_priors, values, index, info=None
    ):
        ...
        name_path = path.join(self.paths.name, self.paths.identifier, "_".join(labels))
...
def make_lists(no_dimensions, step_size, centre_steps=True):
    ...
    sub_lists = make_lists(
        (no_dimensions - 1), step_size[1:], centre_steps=centre_steps
    )
    step_size = step_size[0]
    return [
        (
            [((step_size * value) + ((0.5 * step_size) if centre_steps else 0))]
            + sub_list
        )
