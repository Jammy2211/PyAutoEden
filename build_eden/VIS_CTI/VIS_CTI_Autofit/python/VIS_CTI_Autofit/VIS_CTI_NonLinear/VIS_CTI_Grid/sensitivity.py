...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Parallel import (
    AbstractJob,
    Process,
    AbstractJobResult,
)
...
class JobResult(AbstractJobResult):
    def __init__(self, number, result, perturbed_result):
        """
...
class Sensitivity:
    ...
    def _labels(self):
        ...
        for list_ in self._lists:
            ...
            for (value, prior_tuple) in zip(
                list_, self.perturbation_model.prior_tuples
            ):
    ...
    @property
    def _perturbation_instances(self):
        """
    ...
    def _search_instance(self, name_path):
        """
        ...
        search_instance = self.search.copy_with_paths(paths.for_sub_analysis(name_path))
    ...
    def _make_jobs(self):
        ...
        for (number, (perturbation_instance, search)) in enumerate(
            zip(self._perturbation_instances, self._searches)
        ):
            instance = copy(self.instance)
            instance.perturbation = perturbation_instance
            dataset = self.simulate_function(instance)
            (
                yield self.job_cls(
                    analysis=self.analysis_class(dataset),
                    model=self.model,
