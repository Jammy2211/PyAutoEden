...
from VIS_CTI_Autofit.VIS_CTI_Graphical import (
    EPMeanField,
    MeanField,
    Factor,
    AnalysisFactor,
...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Parallel import SneakyPool
...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Paths.sub_directory_paths import (
    SubDirectoryPaths,
)
from VIS_CTI_Autofit.VIS_CTI_NonLinear.result import Result
from VIS_CTI_Autofit.VIS_CTI_NonLinear.timer import Timer
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Analysis import Analysis
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Paths.null import NullPaths
...
class NonLinearSearch(AbstractFactorOptimiser, ABC):
    def __init__(
        ...
        **kwargs,
        ...
        """
        from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Paths.database import (
            DatabasePaths,
        )
    ...
    def optimise(self, factor, model_approx, status=None):
        """
        ...
        (projection, status) = factor_approx.project(new_model_dist, delta=1)
    ...
    def logger(self):
        ...
        if self._logger is None:
            ...
            if (self.paths is not None) and (self.paths.model is not None):
                log_path = path.join(self.paths.output_path, "output.log")
                os.makedirs(self.paths.output_path, exist_ok=True)
                logger_.handlers.append(logging.FileHandler(log_path))
    ...
    def fit(
        ...
        if not self.paths.is_complete:
            ...
            analysis.save_results_for_aggregator(
                paths=self.paths, model=model, samples=samples
            )
    ...
    @check_cores
    def make_sneaky_pool(self, fitness_function):
        """
        ...
        """
        self.logger.warning(
            "...using SneakyPool. This copies the likelihood functionto each process on instantiation to avoid copying multipletimes."
        )
        return SneakyPool(
