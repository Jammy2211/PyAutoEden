...
import numpy as np
from VIS_CTI_Autofit.VIS_CTI_Plot.samples_plotters import MCMCPlotter
...
class PySwarmsPlotter(MCMCPlotter):
    def contour(self, **kwargs):
        ...
        plotters.plot_contour(pos_history=self.samples.points, **kwargs)
    ...
    def cost_history(self, **kwargs):
        ...
        plotters.plot_cost_history(
            cost_history=self.samples.log_posterior_list, **kwargs
        )
    ...
    def trajectories(self, **kwargs):
        ...
        for i in range(self.samples.model.prior_count):
            ax = axes[i]
            ax.plot(
                np.asarray(points)[:, (-1), i],
                self.samples.log_posterior_list,
                "k",
                alpha=0.3,
            )
