...
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Analysis.multiprocessing import (
    AnalysisPool,
)
...
class Analysis(ABC):
    ...
    def save_results_for_aggregator(self, paths, model, samples):
        pass
...
class CombinedAnalysis(Analysis):
    ...
    def save_results_for_aggregator(self, paths, model, samples):
        def func(child_paths, analysis):
            analysis.save_results_for_aggregator(child_paths, model, samples)
    ...
    def profile_log_likelihood_function(self, paths, instance):
        ...
        def func(child_paths, analysis):
            analysis.profile_log_likelihood_function(child_paths, instance)

        self._for_each_analysis(func, paths)

    def make_result(self, samples, model, search):
        return [
            analysis.make_result(samples, model, search) for analysis in self.analyses
        ]
    ...
    def __add__(self, other):
        ...
        """
        if isinstance(other, CombinedAnalysis):
            return CombinedAnalysis(*self.analyses, *other.analyses)
        return CombinedAnalysis(*self.analyses, other)
