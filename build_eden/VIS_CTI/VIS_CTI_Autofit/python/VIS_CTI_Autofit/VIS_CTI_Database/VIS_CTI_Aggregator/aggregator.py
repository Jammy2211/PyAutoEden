...
from VIS_CTI_Autofit.VIS_CTI_Database.VIS_CTI_Aggregator.scrape import Scraper
from VIS_CTI_Autofit.VIS_CTI_Database import VIS_CTI_Model as m
from VIS_CTI_Autofit.VIS_CTI_Database.VIS_CTI_Query.VIS_CTI_Query import (
    AbstractQuery,
    Attribute,
)
from VIS_CTI_Autofit.VIS_CTI_Database.VIS_CTI_Query.VIS_CTI_Query.attribute import (
    BestFitQuery,
)
...
class Aggregator(AbstractAggregator):
    ...
    def _fits_for_query(self, query):
        """
    ...
    @classmethod
    def from_database(cls, filename, completed_only=False):
        """
        ...
        """
        from VIS_CTI_Autofit.VIS_CTI_Database import open_database
...
class CellAggregator(AbstractAggregator):
    def __init__(self, number, aggregator):
        """
            ...
            for fit in self.aggregator:
                self._fits.append(
                    sorted(
                        fit.children,
                        key=(
                            lambda f: (f.model.order_no if (f.model is not None) else 0)
                        ),
                    )[self.number]
                )
        return self._fits
