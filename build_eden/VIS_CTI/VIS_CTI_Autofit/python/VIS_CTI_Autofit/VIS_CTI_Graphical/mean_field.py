...
class MeanField(CollectionPriorModel, Dict[(Variable, AbstractMessage)], Factor):
    ...
    def __init__(self, dists, log_norm=0.0):
    ...
    def _logpdf(self, **kwargs: np.ndarray):
    ...
    def logpdf(self, values, axis=False):
    ...
    def logpdf_gradient(self, values, axis=False, **kwargs):
        logl = 0
        gradl = {}
        for (v, m) in self.items():
    ...
    def logpdf_gradient_hessian(self, values, axis=False, **kwargs):
        logl = 0.0
        gradl = {}
        hessl = {}
        for (v, m) in self.items():
...
""".join(
    ...
    def prod(self, *approxs: "MeanField"):
        dists = (
    ...
    def _project_mode(self, mode, covar, fun=None):
        """
...
class FactorApproximation(AbstractNode):
    ...
    def func_jacobian(
        ...
        variables=None,
        axis=None,
        _calc_deterministic=True,
        **kwargs,
    ):
        if axis is not None:
            raise NotImplementedError(
                f"FactorApproximation.func_jacobian has not implemeted axis={axis}, try axis=None"
            )
    ...
    def project_mean_field(self, model_dist, delta=1.0, status=None):
