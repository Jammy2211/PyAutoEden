...
from VIS_CTI_Autofit.VIS_CTI_Graphical.utils import aggregate, Axis
from VIS_CTI_Autofit.VIS_CTI_Mapper.variable import Variable


class AbstractFactor(AbstractNode, ABC):
    def __init__(self, name="", **kwargs: Variable):
    ...
    def __mul__(self, other):
        ...
        """
        from VIS_CTI_Autofit.VIS_CTI_Graphical.VIS_CTI_FactorGraphs.graph import (
            FactorGraph,
        )
    ...
    def _resolve_args(self, **kwargs: np.ndarray):
        """
...
class Factor(AbstractFactor):
    ...
    def __init__(
        self, factor, name="", vectorised=False, is_scalar=False, **kwargs: Variable
    ):
        """
    ...
    def calc_exact_projection(self, mean_field):
        if self._calc_exact_projection:
            from VIS_CTI_Autofit.VIS_CTI_Graphical.mean_field import MeanField
    ...
    def safe_exact_update(self, mean_field):
        if self._has_exact_projection:
            from VIS_CTI_Autofit.VIS_CTI_Graphical.mean_field import MeanField
    ...
    def _function_shape(self, **kwargs: np.ndarray):
        """
    ...
    @lru_cache()
    def _var_shape(self, **kwargs: Tuple[(int, ...)]):
        ...
        for (v, vs) in var_shapes.items():
            ...
            if shape.size:
                if not (
                    (np.equal(shape[ind], 1) | np.equal(shape[ind], vshape))
                    | np.equal(vshape, 1)
                ).all():
    ...
    def _call_factor(self, **kwargs: np.ndarray):
        """
        ...
        direct_call = all(
            ((dim == kwargs_dims[k]) for (k, dim) in self._kwargs_dims.items())
        )
        if direct_call:
            return self._factor(**kwargs)
        vectorised = all(
        ...
        if not vectorised:
            raise ValueError(
                f"input dimensions do not match required dimsinput: **kwargs={kwargs_dims}required: **kwargs={self._kwargs_dims}"
            )
    ...
    def broadcast_variable(self, variable, value):
        """
    ...
    def collapse(self, variable, value, agg_func=np.sum):
        """
...
class DeterministicFactor(Factor):
    ...
    def __init__(
        self, factor, variable, *args: Variable, name: str = "", **kwargs: Variable
    ):
        """
        A deterministic factor is used to convert a function f(g(x)) to f(y)g(x) (integrating over y wit
        a delta function) so that it can be represented in a factor graph.
        ...
        kwargs
            Variables for the original factor
        """
        super().__init__(factor, *args, name=(name or factor.__name__), **kwargs)
    ...
    def __call__(self, variable_dict, axis=False):
        ...
        log_val = (
            0.0
            if ((shape == ()) or (axis is None))
            else aggregate(np.zeros(tuple((1 for _ in shape))), axis)
        )
        det_vals = {
