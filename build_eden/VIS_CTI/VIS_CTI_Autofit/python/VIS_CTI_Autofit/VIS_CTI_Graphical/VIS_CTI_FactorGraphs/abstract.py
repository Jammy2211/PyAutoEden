...
from VIS_CTI_Autofit.VIS_CTI_Graphical.utils import FlattenArrays
...
from VIS_CTI_Autofit.VIS_CTI_Graphical.VIS_CTI_FactorGraphs.numerical import (
...
class AbstractNode(ABC):
    ...
    def resolve_variable_dict(self, variable_dict):
        return {
    ...
    def _broadcast(self, plate_inds, value):
        """
    ...
    def _broadcast2d(self, plate_inds, value):
        """
        Ensure the shape of the data matches the shape of the plates
    ...
    def jacobian(
        self, values, variables=None, axis=False, _eps=1e-06, _calc_deterministic=True
    ):
...
class FlattenedNode:
    ...
    def func_jacobian(self, x, axis=None):
        values = self.unflatten(x)
        (fval, jval) = self.node.func_jacobian(values, axis=axis)
        grad = self.flatten(jval)
        return (fval, grad)

    def func_jacobian_hessian(self, x, axis=None):
        values = self.unflatten(x)
