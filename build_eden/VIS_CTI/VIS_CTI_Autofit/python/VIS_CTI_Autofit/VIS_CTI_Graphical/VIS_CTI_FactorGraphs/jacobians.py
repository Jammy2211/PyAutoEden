...
class FactorJacobian(Factor):
    ...
    def __init__(
        ...
        **kwargs: Variable,
        ...
        AbstractFactor.__init__(self, **kwargs, name=(name or factor_jacobian.__name__))

    def __hash__(self):
        return hash(self._factor)

    def _call_factor(self, values, variables=None):
        """
    ...
    def _multicall_factor(self, values, variables=None):
        ...
        if variables is None:
            ...
            njac = len(fjacs[0][1])
            jacs = tuple(
                (np.array([fjac[1][i] for fjac in fjacs]) for i in range(njac))
            )
            return (res, jacs)
    ...
    def func_jacobian(self, variable_dict, variables=None, axis=False, **kwargs):
        """
...
class DeterministicFactorJacobian(FactorJacobian):
    ...
    def __init__(
        ...
        **kwargs: Variable,
        ...
        super().__init__(
            factor_jacobian, vectorised=vectorised, is_scalar=is_scalar, **kwargs
        )
