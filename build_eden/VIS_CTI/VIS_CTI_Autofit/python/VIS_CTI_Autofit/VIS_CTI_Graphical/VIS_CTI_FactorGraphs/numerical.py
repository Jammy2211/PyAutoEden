...
def numerical_func_jacobian(
    factor, values, variables=None, axis=False, _eps=1e-06, _calc_deterministic=True
):
...
def numerical_func_jacobian_hessian(
    ...
    fhess0 = {v: np.empty((f_shape + (np.shape(values[v]) * 2))) for v in variables}
    if _calc_deterministic:
        ...
        if x0.shape:
            ...
            if _calc_deterministic:
                ...
                for (d, d_shape) in det_shapes.items():
                    ...
                    fhess0[d] += (jac[:, None, None] * jac[None, :, None]) * hess[
                        None, None, :
                    ]
