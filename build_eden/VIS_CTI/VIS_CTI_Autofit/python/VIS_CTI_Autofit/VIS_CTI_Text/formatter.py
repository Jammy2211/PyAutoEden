...
from VIS_CTI_Autofit.VIS_CTI_Tools.util import open_
...
class TextFormatter:
    ...
    def add_to_dict(self, path, value, info_dict):
        key = path[0]
        node = info_dict[key]
        if len(path) == 1:
            node.value = value
...
def format_string_for_parameter_name(parameter_name):
    ...
    try:
        for (key, value) in sorted(
            label_conf["format"].items(), key=(lambda item: len(item[0])), reverse=True
        ):
...
def parameter_result_latex_from(
    ...
    if "e" in format_str:
        ...
        parameter_result_latex = f"{psplit[0]}{psplit[1][3:]}{psplit[2][3:]}{psplit[3][(- 1)]} \times 10^{{{int(psplit[1][1:3])}}}"
