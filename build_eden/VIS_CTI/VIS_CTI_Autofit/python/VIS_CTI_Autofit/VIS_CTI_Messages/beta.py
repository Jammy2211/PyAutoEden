...
def inv_beta_suffstats(lnX, ln1X):
    ...
    if np.any((ab < 0)):
        warnings.warn(
            "invalid negative parameters found for inv_beta_suffstats, clampling value to 0.5",
            RuntimeWarning,
        )
...
class BetaMessage(AbstractMessage):
    ...
    def __init__(self, alpha=0.5, beta=0.5, log_norm=0, id_=None):
        self.alpha = alpha
        self.beta = beta
    ...
    def kl(self, dist):
        ...
        return (
            ((betaln(aQ, bQ) - betaln(aP, bP)) - ((aQ - aP) * psi(aP)))
            - ((bQ - bP) * psi(bP))
