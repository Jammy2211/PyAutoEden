...
class MatrixOperator(LinearOperator):
    def __init__(self, M, shape=None, ldim=1):
...
def _mul_triangular(
    ...
    if b.shape[0] != n:
        raise ValueError(
            f"shapes {c.shape} and {b.shape} not aligned: {n} (dim 1) != {b.shape[0]} (dim 0)"
        )
...
class DiagonalMatrix(LinearOperator):
    ...
    def __init__(self, scale, inv_scale=None):
        ...
        self._finv_scale = (1 / scale) if (inv_scale is None) else np.ravel(inv_scale)
    ...
    def from_dense(cls, inverse_hessian):
        ...
        the hessian
        """
        return cls(np.sqrt(np.diagonal(inverse_hessian)))
