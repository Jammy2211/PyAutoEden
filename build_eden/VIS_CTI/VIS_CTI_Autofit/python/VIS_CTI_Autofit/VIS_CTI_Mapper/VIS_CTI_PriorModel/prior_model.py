...
import inspect
import logging
from VIS_CTI_Autoconf.class_path import get_class_path
from VIS_CTI_Autoconf.exc import ConfigException
from VIS_CTI_Autofit.VIS_CTI_Mapper.model import assert_not_frozen
...
from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_PriorModel.abstract import check_assertions
...
class PriorModel(AbstractPriorModel):
    ...
    def __add__(self, other):
        if self.cls != other.cls:
            raise TypeError(
                f"Cannot add PriorModels with different classes ({self.cls.__name__} and {other.cls.__name__})"
            )
        ...
        if key not in (
            ...
            try:
                if "_" in key:
                    name = key.split("_")[0]
                    tuple_prior = [
                        v for (k, v) in self.tuple_prior_tuples if (name == k)
                    ][0]
    ...
    def __getattr__(self, item):
        try:
            if ("_" in item) and (item not in ("_is_frozen", "tuple_prior_tuples")):
                return getattr(
                    [
                        v
                        for (k, v) in self.tuple_prior_tuples
                        if (item.split("_")[0] == k)
                    ][0],
                    item,
                )
