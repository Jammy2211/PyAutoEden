...
from VIS_CTI_Autoconf.exc import ConfigException
from VIS_CTI_Autofit import exc
from VIS_CTI_Autofit.VIS_CTI_Mapper import model
from VIS_CTI_Autofit.VIS_CTI_Mapper.model import AbstractModel, frozen_cache
...
from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_Prior.prior import Limits, GaussianPrior
...
from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_PriorModel.recursion import (
    DynamicRecursionCache,
)
from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_PriorModel.util import PriorModelNameValue
from VIS_CTI_Autofit.VIS_CTI_Text import formatter as frm
from VIS_CTI_Autofit.VIS_CTI_Text.formatter import TextFormatter
from VIS_CTI_Autofit.VIS_CTI_Tools.util import split_paths
...
class AbstractPriorModel(AbstractModel):
    ...
    overridden by any prior model so that the model mapper recognises its prior     model attributes.
    @DynamicAttrs
    """
    ...
    def take_attributes(self, source):
        """
...
{exception}"""
    ...
    def random_unit_vector_within_limits(self, lower_limit=0.0, upper_limit=1.0):
        ...
        """
        return list(
            np.random.uniform(low=lower_limit, high=upper_limit, size=self.prior_count)
        )
    ...
    def random_vector_from_priors_within_limits(self, lower_limit, upper_limit):
        ...
        while True:
            vector = self.vector_from_unit_vector(
                list(
                    np.random.uniform(
                        low=lower_limit, high=upper_limit, size=self.prior_count
                    )
                )
            )
            try:
    ...
    def instance_from_prior_medians(self):
        ...
        """
        return self.instance_from_unit_vector(unit_vector=([0.5] * self.prior_count))
    ...
    def from_instance(instance, model_classes=tuple()):
        ...
        """
        from VIS_CTI_Autofit.VIS_CTI_Mapper.VIS_CTI_PriorModel import collection
    ...
    def __eq__(self, other):
        return isinstance(other, AbstractPriorModel) and (
            self.direct_prior_model_tuples == other.direct_prior_model_tuples
        )
    ...
    def order_no(self):
        ...
        values = [
            ...
            for (_, value) in sorted(
                self.path_instance_tuples_for_class((Prior, float)),
                key=(lambda t: t[0]),
            )
        ]
        return ":".join(values)
    ...
    @property
    def parameter_labels_with_superscripts_latex(self):
        """
    ...
    @property
    def superscripts_overwrite_via_config(self):
        """
