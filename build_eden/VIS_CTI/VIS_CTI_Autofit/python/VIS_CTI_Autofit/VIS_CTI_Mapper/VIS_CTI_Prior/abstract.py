...
def assert_within_limits(func):
    def wrapper(self, *args, ignore_prior_limits=False, **kwargs):
...
class Prior(Variable, ABC, ArithmeticMixin):
    ...
    def __init__(self, lower_limit=0.0, upper_limit=1.0, id_=None):
        """
    ...
    def assert_within_limits(self, value):
        if conf.instance["general"]["model"]["ignore_prior_limits"]:
            return
        if not ((self.lower_limit - epsilon) <= value <= (self.upper_limit + epsilon)):
    ...
    def random(self):
        ...
        """
        return self.value_for(random.random())
