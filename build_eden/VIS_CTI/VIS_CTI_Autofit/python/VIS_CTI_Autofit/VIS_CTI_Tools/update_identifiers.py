...
from VIS_CTI_Autofit.VIS_CTI_Aggregator import Aggregator as ClassicAggregator
from VIS_CTI_Autofit.VIS_CTI_Database.VIS_CTI_Aggregator import (
    Aggregator as DatabaseAggregator,
)
from VIS_CTI_Autofit.VIS_CTI_NonLinear.VIS_CTI_Paths.database import DatabasePaths
from VIS_CTI_Autofit.VIS_CTI_Tools.util import zip_directory

logger = logging.getLogger(__name__)


def update_identifiers_from_file(output_directory, map_filename):
    """
    ...
    update_identifiers_from_dict(output_directory, field_map)


def update_identifiers_from_dict(output_directory, field_map):
    """
    Update identifiers in a given directory by loading and modifying their
    identifier files.
    ...
    """
    aggregator = ClassicAggregator(output_directory)
...
"""
        ...
        new_identifier = md5(".".join(hash_list).encode("utf-8")).hexdigest()
        ...
        if new_identifier == old_identifier:
            logger.warning(f"Skipping {directory} as there is no change")
            zip_file = f"{os.path.split(identifier_filename)[0]}.zip"
            if os.path.exists(zip_file):
                remove_source()
            continue
        new_directory = Path(directory).parent / new_identifier
        print(f"Moving output from {directory} to {new_directory}")
        os.makedirs(new_directory, exist_ok=True)
        for file in os.listdir(directory):
            if file.endswith(".pickle"):
                print(f"Skipping {file}")
                continue
            if not os.path.exists((new_directory / file)):
                shutil.move(f"{directory}/{file}", new_directory)
        with open((new_directory / ".identifier"), "w+") as f:
            f.write(
                """
""".join(
                    hash_list
...
def update_directory_identifiers(output_directory):
    """
    ...
    for output in aggregator:
        ...
        try:
            os.remove(f"{paths.output_path}.zip")
        except FileNotFoundError:
            pass
        ...
        logger.info(f"Moving output from {source_directory} to {target_directory}")
        os.makedirs(target_directory, exist_ok=True)
        for file in os.listdir(source_directory):
            target_filename = f"{target_directory}/{file}"
            if not os.path.exists(target_filename):
                shutil.move(f"{source_directory}/{file}", target_filename)
...
def update_database_identifiers(session):
    ...
    for output in aggregator:
        ...
        paths.search = search
        paths.model = model
        args.append({"old_id": output.id, "new_id": paths.identifier})
    session.execute("UPDATE fit SET id = :new_id WHERE id = :old_id", args)
