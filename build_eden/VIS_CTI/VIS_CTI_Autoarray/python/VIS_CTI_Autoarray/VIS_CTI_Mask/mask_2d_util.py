...
from VIS_CTI_Autoarray.VIS_CTI_Structures.VIS_CTI_Grids.VIS_CTI_TwoD import grid_2d_util


@numba_util.jit()
def mask_2d_centres_from(shape_native, pixel_scales, centre):
    """
...
@numba_util.jit()
def total_pixels_2d_from(mask_2d):
    """
...
@numba_util.jit()
def total_sub_pixels_2d_from(mask_2d, sub_size):
    """
...
@numba_util.jit()
def total_sparse_pixels_2d_from(mask_2d, unmasked_sparse_grid_pixel_centres):
...
@numba_util.jit()
def mask_2d_circular_from(shape_native, pixel_scales, radius, centre=(0.0, 0.0)):
    """
...
def mask_2d_circular_annular_from(
    shape_native, pixel_scales, inner_radius, outer_radius, centre=(0.0, 0.0)
):
    """
...
def mask_2d_circular_anti_annular_from(
    ...
    for y in range(mask_2d.shape[0]):
        for x in range(mask_2d.shape[1]):
            ...
            x_scaled = (x - centres_scaled[1]) * pixel_scales[1]
            r_scaled = np.sqrt(((x_scaled ** 2) + (y_scaled ** 2)))
            if (inner_radius >= r_scaled) or (
                outer_radius_2_scaled >= r_scaled >= outer_radius
            ):
                mask_2d[(y, x)] = False
    return mask_2d


def mask_2d_via_pixel_coordinates_from(shape_native, pixel_coordinates, buffer=0):
    """
...
@numba_util.jit()
def elliptical_radius_from(y_scaled, x_scaled, angle, axis_ratio):
    """
...
def mask_2d_elliptical_from(
    shape_native, pixel_scales, major_axis_radius, axis_ratio, angle, centre=(0.0, 0.0)
):
    """
...
def mask_2d_elliptical_annular_from(
    ...
    for y in range(mask_2d.shape[0]):
        for x in range(mask_2d.shape[1]):
            ...
            if (inner_r_scaled_elliptical >= inner_major_axis_radius) and (
                outer_r_scaled_elliptical <= outer_major_axis_radius
            ):
                mask_2d[(y, x)] = False
    return mask_2d
...
@numba_util.jit()
def blurring_mask_2d_from(mask_2d, kernel_shape_native):
    """
            ...
            if not mask_2d[(y, x)]:
                for y1 in range(
                    ...
                    for x1 in range(
                        ...
                        if (0 <= (x + x1) <= (mask_2d.shape[1] - 1)) and (
                            0 <= (y + y1) <= (mask_2d.shape[0] - 1)
                        ):
                            if mask_2d[((y + y1), (x + x1))]:
                                blurring_mask_2d[((y + y1), (x + x1))] = False
                        else:
                            raise exc.MaskException(
                                "setup_blurring_mask extends beyond the sub_size of the mask - pad the datas array before masking"
                            )
    return blurring_mask_2d


@numba_util.jit()
def mask_2d_via_shape_native_and_native_for_slim(shape_native, native_for_slim):
    """
...
@numba_util.jit()
def total_edge_pixels_from(mask_2d):
    """
...
@numba_util.jit()
def edge_1d_indexes_from(mask_2d):
    """
...
@numba_util.jit()
def check_if_border_pixel(mask_2d, edge_pixel_slim, native_to_slim):
    """
...
@numba_util.jit()
def border_slim_indexes_from(mask_2d):
    """
...
def sub_border_pixel_slim_indexes_from(mask_2d, sub_size):
    """
...
@numba_util.jit()
def buffed_mask_2d_from(mask_2d, buffer=1):
    """
...
def rescaled_mask_2d_from(mask_2d, rescale_factor):
    """
...
@numba_util.jit()
def slim_index_for_sub_slim_index_via_mask_2d_from(mask_2d, sub_size):
    """ "
...
def sub_slim_indexes_for_slim_index_via_mask_2d_from(mask_2d, sub_size):
    """ "
...
@numba_util.jit()
def sub_slim_index_for_sub_native_index_from(sub_mask_2d):
    """
...
@numba_util.jit()
def native_index_for_slim_index_2d_from(mask_2d, sub_size):
    """
...
@numba_util.jit()
def mask_2d_neighbors_from(mask_2d):
    """
