...
from VIS_CTI_Autoarray.VIS_CTI_Dataset.abstract_dataset import AbstractWTilde
from VIS_CTI_Autoarray.VIS_CTI_Dataset.abstract_dataset import AbstractSettingsDataset
from VIS_CTI_Autoarray.VIS_CTI_Dataset.abstract_dataset import AbstractDataset
...
class Imaging(AbstractDataset):
    def __init__(
        ...
        if pad_for_convolver and (psf is not None):
            try:
                ...
                logger.info(
                    f"The image and noise map of the `Imaging` objected have been padded to the dimensions{image.shape}. This is because the blurring region around the mask (which defines wherePSF flux may be convolved into the masked region) extended beyond the edge of the image.This can be prevented by using a smaller mask, smaller PSF kernel size or manually paddingthe image and noise-map yourself."
                )
    ...
    def signal_to_noise_limited_from(self, signal_to_noise_limit, mask=None):
        ...
        noise_map_limit = np.where(
            (
                (self.signal_to_noise_map.native > signal_to_noise_limit)
                & (mask == False)
            ),
