    ...
    def check_noise_map(self, noise_map):
        if noise_map[0] != self.noise_map_value:
            raise exc.InversionException(
                f"The preloaded values of WTildeImaging are not consistent with the noise-map passed to them, thus they cannot be used for the inversion.The value of the noise map is {noise_map[0]} whereas in WTildeImaging it is {self.noise_map_value}Update WTildeImaging or do not use the w_tilde formalism to perform the LEq."
            )


def grid_via_grid_class_from(
    mask, grid_class, fractional_accuracy, relative_accuracy, sub_steps
):
...
class AbstractSettingsDataset:
    ...
    def grid_inversion_from(self, mask):
        return grid_via_grid_class_from(
            mask=mask,
...
class AbstractDataset:
    def __init__(self, data, noise_map, settings=AbstractSettingsDataset(), name=None):
        """
