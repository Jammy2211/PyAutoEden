...
class Interferometer(AbstractDataset):
    ...
    def uv_distances(self):
        return np.sqrt(
            (
                np.square(self.uv_wavelengths[:, 0])
                + np.square(self.uv_wavelengths[:, 1])
            )
        )
...
class AbstractSimulatorInterferometer:
    def __init__(
        ...
        real_space_shape_native
            The shape of the observation. Note that we do not simulator a full Imaging array (e.g. 2000 x 2000 pixels for             Hubble imaging), but instead just a cut-out around the strong lens.
