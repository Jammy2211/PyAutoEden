...
class Preloads:
    ...
    def set_w_tilde_imaging(self, fit_0, fit_1):
        ...
        if (fit_0.inversion is not None) and (
            np.max(abs((fit_0.noise_map - fit_1.noise_map))) < 1e-08
        ):
            ...
            (
                ...
                indexes,
                lengths,
            ) = leq_util.w_tilde_curvature_preload_imaging_from(
        ...
        if (
            ...
            if (
                ...
                if isinstance(inversion_0.leq, AbstractLEqImaging):
                    self.curvature_matrix_preload = inversion_0.curvature_matrix_preload.astype(
                        "int"
                    )
                    self.curvature_matrix_counts = inversion_0.curvature_matrix_counts.astype(
                        "int"
                    )
                logger.info(
    ...
    def check_via_fit(self, fit):
        ...
        if abs((fom_with_preloads - fom_without_preloads)) > 0.0001:
            raise exc.PreloadsException(
                f"The log likelihood of fits using and not using preloads are notconsistent, indicating preloading has gone wrong.The likelihood values are {fom_with_preloads} (with preloads) and {fom_without_preloads} (without preloads)"
            )
    ...
    def info(self):
        ...
        """
        line = [
            f"""W Tilde = {(self.w_tilde is not None)}
"""
        ]
        line += [
            f"""Relocated Grid = {(self.relocated_grid is not None)}
"""
        ]
        line += [
            f"""Mapper = {(self.linear_obj_list is not None)}
"""
        ]
        line += [
...
"""
        ...
        line += [
            f"""Regularization Matrix = {(self.regularization_matrix is not None)}
"""
        ]
        line += [
