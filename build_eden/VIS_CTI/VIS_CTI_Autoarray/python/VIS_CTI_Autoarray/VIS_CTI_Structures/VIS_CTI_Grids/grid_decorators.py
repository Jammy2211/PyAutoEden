...
from VIS_CTI_Autoarray.VIS_CTI_Structures.VIS_CTI_Grids.VIS_CTI_OneD.abstract_grid_1d import (
    AbstractGrid1D,
)
...
def grid_1d_to_structure(func):
    ...
    @wraps(func)
    def wrapper(obj, grid, radial_grid_shape_slim=None, *args, **kwargs):
        """
...
def transform(func):
    """
    Checks whether the input Grid2D of (y,x) coordinates have previously been transformed. If they have not     been transformed then they are transformed.
...
def relocate_to_radial_minimum(func):
    """ Checks whether any coordinates in the grid are radially near (0.0, 0.0), which can lead to numerical faults in     the evaluation of a function (e.g. numerical integration reaching a singularity at (0.0, 0.0)). If any coordinates
    ...
    def wrapper(cls, grid, *args, **kwargs):
        ...
        with np.errstate(all="ignore"):
            ...
            grid_radial_scale = np.where(
                (grid_radii < grid_radial_minimum),
                (grid_radial_minimum / grid_radii),
                1.0,
            )
