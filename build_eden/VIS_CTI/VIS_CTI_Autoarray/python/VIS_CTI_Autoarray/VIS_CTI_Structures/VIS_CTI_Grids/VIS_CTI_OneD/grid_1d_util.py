...
from VIS_CTI_Autoarray.VIS_CTI_Mask import mask_1d_util


def grid_1d_slim_via_shape_slim_from(shape_slim, pixel_scales, sub_size, origin=(0.0,)):
    """
...
@numba_util.jit()
def grid_1d_slim_via_mask_from(mask_1d, pixel_scales, sub_size, origin=(0.0,)):
    """
    For a sub-grid, every unmasked pixel of its 1D mask with shape (total_pixels,) is divided into a finer uniform
    grid of shape (total_pixels*sub_size, ). This routine computes the (x) scaled coordinates at the centre of every
    ...
    for x in range(mask_1d.shape[0]):
        if not mask_1d[x]:
            ...
            for x1 in range(sub_size):
                grid_1d[sub_index] = ((x_scaled - x_sub_half) + (x1 * x_sub_step)) + (
                    x_sub_step / 2.0
                )
                sub_index += 1
    return grid_1d
...
def grid_1d_slim_from(grid_1d_native, mask_1d, sub_size):
    """
...
def grid_1d_native_from(grid_1d_slim, mask_1d, sub_size):
    """
