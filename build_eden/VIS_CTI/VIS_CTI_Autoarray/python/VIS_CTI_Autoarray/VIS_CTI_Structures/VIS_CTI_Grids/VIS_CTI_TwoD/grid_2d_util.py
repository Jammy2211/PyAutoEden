...
from VIS_CTI_Autoarray.VIS_CTI_Geometry import geometry_util
from VIS_CTI_Autoarray import numba_util
from VIS_CTI_Autoarray.VIS_CTI_Mask import mask_2d_util
...
@numba_util.jit()
def grid_2d_slim_via_mask_from(mask_2d, pixel_scales, sub_size, origin=(0.0, 0.0)):
    """
                ...
                for y1 in range(sub_size):
                    for x1 in range(sub_size):
                        grid_slim[(sub_index, 0)] = -(
                            ((y_scaled - y_sub_half) + (y1 * y_sub_step))
                            + (y_sub_step / 2.0)
                        )
                        grid_slim[(sub_index, 1)] = (
                            (x_scaled - x_sub_half) + (x1 * x_sub_step)
                        ) + (x_sub_step / 2.0)
                        sub_index += 1
    return grid_slim


def grid_2d_via_mask_from(mask_2d, pixel_scales, sub_size, origin=(0.0, 0.0)):
    """
...
def grid_2d_slim_via_shape_native_from(
    shape_native, pixel_scales, sub_size, origin=(0.0, 0.0)
):
    """
    For a sub-grid, every unmasked pixel of its 2D mask with shape (total_y_pixels, total_x_pixels) is divided into a
    finer uniform grid of shape (total_y_pixels*sub_size, total_x_pixels*sub_size). This routine computes the (y,x)
...
def grid_scaled_2d_slim_radial_projected_from(
    extent, centre, pixel_scales, sub_size, shape_slim=0
):
    """
...
def grid_pixels_2d_slim_from(
    grid_scaled_2d_slim, shape_native, pixel_scales, origin=(0.0, 0.0)
):
    """
    ...
    for slim_index in range(grid_scaled_2d_slim.shape[0]):
        ...
        grid_pixels_2d_slim[(slim_index, 1)] = (
            (grid_scaled_2d_slim[(slim_index, 1)] / pixel_scales[1]) + centres_scaled[1]
        ) + 0.5
...
def grid_scaled_2d_slim_from(
    grid_pixels_2d_slim, shape_native, pixel_scales, origin=(0.0, 0.0)
):
    """
...
def grid_pixel_centres_2d_from(
    grid_scaled_2d, shape_native, pixel_scales, origin=(0.0, 0.0)
):
    """
    ...
    for y in range(grid_scaled_2d.shape[0]):
        for x in range(grid_scaled_2d.shape[1]):
            ...
            grid_pixels_2d[(y, x, 1)] = int(
                (
                    ((grid_scaled_2d[(y, x, 1)] / pixel_scales[1]) + centres_scaled[1])
                    + 0.5
                )
            )
...
def grid_2d_slim_from(grid_2d_native, mask, sub_size):
    """
...
def grid_2d_native_from(grid_2d_slim, mask_2d, sub_size):
    """
