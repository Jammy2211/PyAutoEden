...
from VIS_CTI_Autoarray.VIS_CTI_Structures.abstract_structure import AbstractStructure2D
...
def check_grid_2d_and_mask_2d(grid_2d, mask_2d):
    if len(grid_2d.shape) == 2:
        if grid_2d.shape[0] != mask_2d.sub_pixels_in_mask:
            raise exc.GridException(
                "The input 1D grid does not have the same number of entries as sub-pixels inthe mask."
            )
    elif len(grid_2d.shape) == 3:
        if (grid_2d.shape[0], grid_2d.shape[1]) != mask_2d.sub_shape_native:
            raise exc.GridException(
                "The input grid is 2D but not the same dimensions as the sub-mask (e.g. the mask 2D shape multipled by its sub size.)"
            )
...
class AbstractGrid2D(AbstractStructure2D):
    ...
    def grid_2d_radial_projected_from(
        self, centre=(0.0, 0.0), angle=0.0, radial_grid_shape_slim=None
    ):
        """
    ...
    def extent_with_buffer(self, buffer=1e-08):
        """
