...
from VIS_CTI_Autoarray.VIS_CTI_Structures.VIS_CTI_Grids.VIS_CTI_TwoD.abstract_grid_2d import (
    AbstractGrid2D,
)
...
from VIS_CTI_Autoarray.VIS_CTI_Structures.VIS_CTI_Grids import abstract_grid
...
from VIS_CTI_Autoarray import type as ty
...
class Grid2DIterate(AbstractGrid2D):
    ...
    @classmethod
    def from_mask(
        cls, mask, fractional_accuracy=0.9999, relative_accuracy=None, sub_steps=None
    ):
        """
    ...
    def threshold_mask_via_arrays_from(self, array_lower_sub_2d, array_higher_sub_2d):
        """
                ...
                for x in range(threshold_mask.shape[1]):
                    if not array_higher_mask[(y, x)]:
                        if (
                            abs(
                                (
                                    array_lower_sub_2d[(y, x)]
                                    - array_higher_sub_2d[(y, x)]
                                )
                            )
                            > relative_accuracy_threshold
                        ):
    ...
    def return_iterated_array_result(self, iterated_array):
        """
    ...
    def iterated_array_jit_from(
        ...
        for y in range(iterated_array.shape[0]):
            for x in range(iterated_array.shape[1]):
                if threshold_mask_higher_sub[(y, x)] and (
                    not threshold_mask_lower_sub[(y, x)]
                ):
                    iterated_array[(y, x)] = array_higher_sub_2d[(y, x)]
        return iterated_array

    def threshold_mask_via_grids_from(self, grid_lower_sub_2d, grid_higher_sub_2d):
        """
        Returns a fractional mask from a result array, where the fractional mask describes whether the evaluated
        value in the result array is within the `Grid2DIterate`'s specified fractional accuracy. The fractional mask thus
        determines whether a pixel on the grid needs to be reevaluated at a higher level of sub-gridding to meet the
                ...
                for x in range(threshold_mask.shape[1]):
                    if not grid_higher_mask[(y, x)]:
                        relative_accuracy_y = abs(
                            (
                                grid_lower_sub_2d[(y, x, 0)]
                                - grid_higher_sub_2d[(y, x, 0)]
                            )
                        )
                        relative_accuracy_x = abs(
                            (
                                grid_lower_sub_2d[(y, x, 1)]
                                - grid_higher_sub_2d[(y, x, 1)]
                            )
                        )
