...
def curvature_reg_matrix_from(
    curvature_matrix, regularization_matrix, pixel_neighbors, pixel_neighbors_sizes
):
    """
...
def reconstruction_from(
    data_vector, curvature_reg_matrix_cholesky, settings=SettingsInversion()
):
    """
    ...
    if settings.check_solution:
        if np.isclose(a=reconstruction[0], b=reconstruction[1], atol=0.0001).all():
            if np.isclose(a=reconstruction[0], b=reconstruction, atol=0.0001).all():
                raise exc.InversionException()
