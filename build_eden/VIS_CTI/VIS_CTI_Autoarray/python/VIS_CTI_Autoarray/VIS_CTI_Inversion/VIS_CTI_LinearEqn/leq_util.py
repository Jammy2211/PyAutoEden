...
def w_tilde_data_imaging_from(
    image_native, noise_map_native, kernel_native, native_index_for_slim_index
):
    """
        ...
        for k0_y in range(kernel_native.shape[0]):
            for k0_x in range(kernel_native.shape[1]):
                weight_value = weight_map_native[
                    (
                        ((ip0_y + k0_y) + kernel_shift_y),
                        ((ip0_x + k0_x) + kernel_shift_x),
                    )
                ]
...
def w_tilde_curvature_imaging_from(
    noise_map_native, kernel_native, native_index_for_slim_index
):
    """
...
def w_tilde_curvature_value_from(
    value_native, kernel_native, ip0_y, ip0_x, ip1_y, ip1_x, renormalize=False
):
    """
        ...
        for k0_x in range(kernel_native.shape[1]):
            ...
            if value > 0.0:
                ...
                if (
                    ...
                    curvature_value += (kernel_value_0 * kernel_value_1) * (
                        (1.0 / value) ** 2.0
                    )
    if renormalize:
        if kernel_count > 0:
...
def data_vector_via_w_tilde_data_imaging_from(
    w_tilde_data, data_to_pix_unique, data_weights, pix_lengths, pix_pixels
):
    """
    ...
    for data_index in range(data_shape[0]):
        for pix_index in range(data_shape[1]):
            data_vector[pix_index] += (
                image[data_index] * blurred_mapping_matrix[(data_index, pix_index)]
...
@numba_util.jit()
def curvature_matrix_via_w_tilde_from(w_tilde, mapping_matrix):
    """
...
def curvature_matrix_off_diags_via_w_tilde_curvature_preload_imaging_from(
    ...
    curvature_indexes,
    curvature_lengths,
    data_to_pix_unique_0,
    data_weights_0,
    pix_lengths_0,
    pix_pixels_0,
    data_to_pix_unique_1,
    data_weights_1,
    pix_lengths_1,
    pix_pixels_1,
):
...
@numba_util.jit()
def curvature_matrix_preload_from(mapping_matrix, mapping_matrix_threshold=1e-08):
    """
...
def curvature_matrix_via_sparse_preload_from(
    mapping_matrix, noise_map, curvature_matrix_preload, curvature_matrix_counts
):
    """
    Returns the curvature matrix `F` from a blurred mapping matrix `f` and the 1D noise-map $\\sigma$
...
def mapped_reconstructed_data_via_image_to_pix_unique_from(
    data_to_pix_unique, data_weights, pix_lengths, reconstruction
):
    """
...
@numba_util.jit()
def mapped_reconstructed_data_via_mapping_matrix_from(mapping_matrix, reconstruction):
    """
    Returns the reconstructed data vector from the blurred mapping matrix `f` and solution vector *S*.
...
@numba_util.jit()
def mapped_reconstructed_visibilities_from(transformed_mapping_matrix, reconstruction):
    """
    ...
    """
    mapped_reconstructed_visibilities = (0.0 + 0j) * np.zeros(
    ...
    for i in range(transformed_mapping_matrix.shape[0]):
        for j in range(reconstruction.shape[0]):
            ...
            ) + (1j * (reconstruction[j] * transformed_mapping_matrix_imag[(i, j)]))
