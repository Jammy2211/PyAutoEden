...
from os import path
import os
from VIS_CTI_Autoarray.VIS_CTI_Structures.abstract_structure import AbstractStructure
...
class Figure(AbstractMatWrap):
    ...
    def aspect_from(self, shape_native):
        """
...
class Axis(AbstractMatWrap):
    def __init__(self, symmetric_source_centre=False, **kwargs):
        """
...
class Colorbar(AbstractMatWrap):
    def __init__(self, manual_tick_labels=None, manual_tick_values=None, **kwargs):
        """
    ...
    def set(self):
        ...
        elif (self.manual_tick_values is not None) and (
            self.manual_tick_labels is not None
        ):
            cb = plt.colorbar(ticks=self.manual_tick_values, **self.config_dict)
            cb.ax.set_yticklabels(labels=self.manual_tick_labels)
        else:
            raise exc.PlottingException(
                "Only 1 entry of tick_values or tick_labels was input. You must either supplyboth the values and labels, or neither."
            )
        return cb

    def set_with_color_values(self, cmap, color_values):
        """
...
class AbstractTicks(AbstractMatWrap):
    def __init__(self, manual_values=None, **kwargs):
        """
    ...
    def tick_values_from(self, min_value, max_value):
        """
    ...
    def tick_values_in_units_from(self, array, min_value, max_value, units):
        """
        ...
        elif (units.use_scaled and (units.conversion_factor is None)) or (
            not units.in_kpc
        ):
...
class YTicks(AbstractTicks):
    def set(self, array, min_value, max_value, units):
        """
...
class XTicks(AbstractTicks):
    def set(self, array, min_value, max_value, units, use_integers=False):
        """
...
class YLabel(AbstractLabel):
    def set(self, units, include_brackets=True, auto_label=None):
        """
...
class Output:
    def __init__(
        self, path=None, filename=None, format=None, format_folder=False, bypass=False
    ):
        """
