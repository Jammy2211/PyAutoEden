...
class Extractor1DFPR(Extractor1D):
    def array_1d_list_from(self, array, pixels):
        """
    ...
    def stacked_array_1d_from(self, array, pixels):
    ...
    def region_list_from(self, pixels):
        ...
        return list(
            map(
                (lambda region: region.front_region_from(pixels=pixels)),
                self.region_list,
            )
        )
...
class AbstractLayout1DLine(Layout1D):
    def __init__(
        ...
        """
        Abstract base class for a charge injection layout_ci, which defines the regions charge injections appears          on a charge-injection array, the input normalization and other properties.
        ...
        region_list: [(int,)]
            A list of the integer coordinates specifying the corners of each charge injection region             (top-row, bottom-row, left-column, right-column).
        """
        self.region_list = list(map(Region1D, region_list))
    ...
    def array_1d_of_regions_from(self, array):
        """
    ...
    def array_1d_of_non_regions_from(self, array):
        """
    ...
    def array_1d_of_trails_from(self, array):
        """
    ...
    def array_1d_of_edges_and_epers_from(self, array, fpr_range=None, trails_rows=None):
        """
