...
from VIS_CTI_Autoarray.VIS_CTI_Mask import mask_1d
from VIS_CTI_Autoarray.VIS_CTI_Dataset import abstract_dataset
from VIS_CTI_Autoarray.VIS_CTI_Structures.VIS_CTI_Arrays.VIS_CTI_OneD.array_1d import (
    Array1D,
)
from VIS_CTI_Autoarray.VIS_CTI_Dataset.imaging import AbstractSimulatorImaging
...
class DatasetLine(abstract_dataset.AbstractDataset):
    def __init__(
        self, data, noise_map, pre_cti_data, layout, settings=SettingsDatasetLine()
    ):
    ...
    def from_fits(
        ...
        elif isinstance(layout, Layout1DLine):
...
class SimulatorDatasetLine(AbstractSimulatorImaging):
    ...
    def from_layout(self, layout, clocker, trap_list=None, ccd=None):
        ...
        frame_geometry : CIQuadGeometry
            The quadrant geometry of the simulated data, defining where the parallel / serial overscans are and             therefore the direction of clocking and rotations before input into the cti algorithm.
    ...
    def from_pre_cti_data(
        self, pre_cti_data, layout, clocker, trap_list=None, ccd=None
    ):
    ...
    def from_post_cti_data(self, post_cti_data, pre_cti_data, layout):
