...
from VIS_CTI_Autocti.VIS_CTI_ChargeInjection.VIS_CTI_Extractor2D.parallel_fpr import (
    Extractor2DParallelFPR,
)
from VIS_CTI_Autocti.VIS_CTI_ChargeInjection.VIS_CTI_Extractor2D.parallel_eper import (
    Extractor2DParallelEPER,
)
from VIS_CTI_Autocti.VIS_CTI_ChargeInjection.VIS_CTI_Extractor2D.serial_fpr import (
    Extractor2DSerialFPR,
)
from VIS_CTI_Autocti.VIS_CTI_ChargeInjection.VIS_CTI_Extractor2D.serial_eper import (
    Extractor2DSerialEPER,
)
...
class AbstractLayout2DCI(aa.Layout2D):
    ...
    @property
    def parallel_rows_to_array_edge(self):
        """
    ...
    @property
    def smallest_parallel_rows_between_ci_regions(self):
        """
    ...
    def array_2d_of_regions_from(self, array):
        """
    ...
    def array_2d_of_non_regions_from(self, array):
        """
    ...
    def array_2d_of_parallel_epers_from(self, array):
        """
    ...
    def array_2d_of_parallel_fprs_and_epers_from(
        self, array, fpr_range=None, trails_rows=None
    ):
        """
    ...
    def extraction_region_for_parallel_calibration_from(self, columns):
        """
    ...
    def array_2d_for_parallel_calibration_from(self, array, columns):
        """
        Extract a parallel calibration array from an input array, where this array contains a sub-set of the input
        array which is specifically used for only parallel CTI calibration. This array is simply a specified number
        of columns that are closest to the read-out electronics.
    ...
    def extracted_layout_for_parallel_calibration_from(self, columns):
        """
        Extract a parallel calibration array from an input array, where this array contains a sub-set of the input
        array which is specifically used for only parallel CTI calibration. This array is simply a specified number
        of columns that are closest to the read-out electronics.
    ...
    def array_2d_of_serial_trails_from(self, array):
    ...
    def array_2d_of_serial_overscan_above_trails_from(self, array):
        """
    ...
    def array_2d_list_for_serial_calibration(self, array):
        """
    ...
    def array_2d_for_serial_calibration_from(self, array, rows):
        """
    ...
    def mask_for_serial_calibration_from(self, mask, rows):
        """
        Extract a serial calibration array from a charge injection array, where this arrays is a sub-set of the
        array which can be used for serial-only calibration. Specifically, this array is all charge injection
        scans and their serial over-scan trails.
    ...
    def extract_line_from(self, array, line_region):
...
class Layout2DCI(AbstractLayout2DCI):
    """
    A uniform charge injection layout_ci, which is defined by the regions it appears on the charge injection     array and its normalization.
    """
    ...
    def pre_cti_data_from(self, shape_native, pixel_scales):
        """Use this charge injection layout_ci to generate a pre-cti charge injection image. This is performed by         going to its charge injection regions and adding the charge injection normalization value.
...
class Layout2DCINonUniform(AbstractLayout2DCI):
    ...
    def region_ci_from(self, region_dimensions, ci_seed):
        """Generate the non-uniform charge distribution of a charge injection region. This includes non-uniformity         across both the rows and columns of the charge injection region.

        Before adding non-uniformity to the rows and columns, we assume an input charge injection level         (e.g. the average current being injected). We then simulator non-uniformity in this region.

        Non-uniformity in the columns is caused by sharp peaks and troughs in the input charge current. To simulator          this, we change the normalization of each column by drawing its normalization value from a Gaussian         distribution which has a mean of the input normalization and standard deviation *column_sigma*. The seed         of the random number generator ensures that the non-uniform charge injection update_via_regions of each pre_cti_datas         are identical.

        Non-uniformity in the rows is caused by the charge smoothly decreasing as the injection is switched off. To         simulator this, we assume the charge level as a function of row number is not flat but defined by a         power-law with slope *row_slope*.

        Non-uniform charge injection images are generated using the function *simulate_pre_cti*, which uses this         function.
        ...
        for column_number in range(ci_columns):
            ...
            while (column_normalization <= 0) or (
                column_normalization >= self.maximum_normalization
            ):
    ...
    def pre_cti_data_from(self, shape_native, pixel_scales, ci_seed=(-1)):
        """Use this charge injection layout_ci to generate a pre-cti charge injection image. This is performed by going         to its charge injection regions and adding its non-uniform charge distribution.

        For one column of a non-uniform charge injection pre_cti_datas, it is assumed that each non-uniform charge         injection region has the same overall normalization value (after drawing this value randomly from a Gaussian         distribution). Physically, this is true provided the spikes / troughs in the current that cause         non-uniformity occur in an identical fashion for the generation of each charge injection region.
        ...
        ci_seed : int
            Input ci_seed for the random number generator to give reproducible results. A new ci_seed is always used for each             pre_cti_datas, ensuring each non-uniform ci_region has the same column non-uniformity layout_ci.
        """
        pre_cti_data = np.zeros(shape_native)
        if ci_seed == (-1):
            ci_seed = np.random.randint(0, int(1000000000.0))
        for region in self.region_list:
