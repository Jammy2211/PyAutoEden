...
class AbstractSettingsCTI:
    def check_total_density_within_range_of_traps(self, total_density_range, traps):
        if total_density_range is not None:
            total_density = sum([trap.density for trap in traps])
            if (total_density < total_density_range[0]) or (
                total_density > total_density_range[1]
            ):
