...
from SLE_Model_Autoconf.class_path import get_class_path, get_class
...
def nd_array_from_dict(nd_array_dict, **_):
    """
...
def compound_key_dict(obj):
    ...
    return {
        "type": "compound_dict",
        "arguments": [
            {"key": to_dict(key), "value": to_dict(value)}
            for (key, value) in obj.items()
        ],
    }


def to_dict(obj, filter_args=()):
    ...
    if isinstance(obj, np.number):
        return {"type": "np.number", "dtype": str(obj.dtype), "value": obj.item()}
...
def instance_as_dict(obj, filter_args=()):
    """
    ...
    argument_dict = {
        ...
        if (hasattr(obj, arg) and (not inspect.ismethod(getattr(obj, arg))))
    }
    try:
...
def register_parser(type_, parser):
    """
...
def from_dict(dictionary, **kwargs):
    ...
    if type_ == "np.number":
        return getattr(np, dictionary["dtype"])(dictionary["value"])
    if type_ == "function":
...
def output_to_json(obj, file_path):
    """
