...
class EPMeanField(FactorGraph):
    ...
    def merge(self, index, subset_approx):
        ...
        return type(self)(self.factor_graph, factor_mean_field)

    def update_factor_mean_field(self, factor, new_dist, index=None):
        if index:
    ...
    def factor_approximation(self, factor):
        ...
        cavity_dist = MeanField({v: 1.0 for v in factor_dist.all_variables}).prod(
            *factor_mean_field.values(), default=factor_dist
        )
    ...
    def project_mean_field(self, new_dist, factor_approx, delta=1.0, status=Status()):
    ...
    @property
    def variable_message_count(self):
        """
        ...
        return {
            ...
            for (variable, messages) in self.variable_messages.items()
        }
...
class EPMeanFieldSubset(EPMeanField):
    ...
    def project_mean_field(
        self, new_model_dist, factor_approx, delta=1.0, status=Status()
    ):
        factor_mean_field = self.factor_mean_field
        rescale1 = {
            v: (1 - scale)
        ...
        if delta < 1:
            ...
            factor_mean_field[factor_approx.factor] = (
                new_factor_dist * subset_cavity_dist
            )
        new_approx = type(self)(
            factor_graph=self._factor_graph,
