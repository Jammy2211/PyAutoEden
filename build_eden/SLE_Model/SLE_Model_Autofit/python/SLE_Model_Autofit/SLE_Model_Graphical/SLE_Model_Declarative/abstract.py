...
from SLE_Model_Autofit.SLE_Model_Graphical.SLE_Model_Declarative.SLE_Model_Factor.prior import (
    PriorFactor,
)
from SLE_Model_Autofit.SLE_Model_Graphical.SLE_Model_Declarative.graph import (
...
class AbstractDeclarativeFactor(Analysis, ABC):
    ...
    def message_dict(self):
        ...
        return {
            prior: (
                (prior.message ** (1 / (count - 1))) if (count > 1) else prior.message
            )
    ...
    def _make_ep_optimiser(self, optimiser, paths=None, ep_history=None):
        return EPOptimiser(
            self.graph,
    ...
    def optimise(self, optimiser, paths=None, ep_history=None, **kwargs):
        """
        ...
        """
        from SLE_Model_Autofit.SLE_Model_Graphical.SLE_Model_Declarative.result import (
            EPResult,
        )
