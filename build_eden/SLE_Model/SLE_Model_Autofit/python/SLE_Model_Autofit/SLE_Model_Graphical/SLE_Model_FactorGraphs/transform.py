...
class TransformedNode(AbstractNode):
    ...
    def func_jacobian(self, values, variables=None, _calc_deterministic=True, **kwargs):
    ...
    def func_jacobian_hessian(
        self, values, variables=None, _calc_deterministic=True, **kwargs
    ):
        M = self.transform
        (fval, jval, hval) = self.node.func_jacobian_hessian(
            M.ldiv(values), variables=variables, _calc_deterministic=_calc_deterministic
        )
        grad = jval / M
        hess = M.invquad(hval)
