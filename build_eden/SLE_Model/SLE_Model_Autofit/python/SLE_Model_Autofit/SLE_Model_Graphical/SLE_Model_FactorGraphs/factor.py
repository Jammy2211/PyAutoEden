...
from SLE_Model_Autofit.SLE_Model_Mapper.variable import Variable, Plate, VariableData
...
class Factor(AbstractFactor):
    ...
    *args: Variables
    ...
    def __init__(
        ...
        *args: Variable,
        ...
        AbstractFactor.__init__(
            self, name=name, plates=plates, factor_out=factor_out, **kwargs
        )
        self.eps = eps
    ...
    def __getitem__(self, plates_index):
        return self.subset(plates_index)

    def subset(self, plates_index, plate_sizes=None):
        if not self.plates:
            return self
    ...
    def _jax_factor_vjp(self, *args):
        return jax.vjp(self._factor, *args)

    _factor_vjp = _jax_factor_vjp

    def _vjp_func_jacobian(self, values):
        ...
        fvjp_op = VectorJacobianProduct(
            self.factor_out, fvjp, *self.args, out_shapes=fval.to_dict().shapes
        )
        return (fval, fvjp_op)
    ...
    def _jvp_func_jacobian(self, values, **kwargs):
        args = self.resolve_args(values)
    ...
    def _jac_out_to_jvp(self, raw_jac, values):
        from SLE_Model_Autofit.SLE_Model_Graphical.SLE_Model_FactorGraphs.jacobians import (
            JacobianVectorProduct,
        )
