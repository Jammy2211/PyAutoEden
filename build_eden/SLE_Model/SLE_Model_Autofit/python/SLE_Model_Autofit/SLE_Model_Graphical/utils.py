...
from SLE_Model_Autofit.SLE_Model_Mapper.variable import Variable, VariableData
...
class LogWarnings(warnings.catch_warnings):
    def __init__(
        self, *, module=None, messages=None, action=None, logger=logging.warning
    ):
...
def is_iterable(arg):
    return isinstance(arg, abc.Iterable) and (not isinstance(arg, six.string_types))
...
def nested_map(func, *args):
    ...
    if isinstance(out, dict):
        return type(out)(
            {k: nested_map(func, *(arg[k] for arg in args)) for k in sorted(out)}
        )
...
def to_variabledata(variables, raw_values):
...
class FlattenArrays(dict):
    ...
    def __init__(self, dict_):
        ...
        self.inds = [
            slice(i0, i1)
            for (i0, i1) in zip(np.r_[(0, self.splits[:(-1)])], self.splits)
        ]
    ...
    def unflatten(self, arr, ndim=None):
        ...
        return VariableData(
            {
                k: (
                    arr.reshape(((shape * ndim) + arr_shape))
                    if (shape or arr_shape)
                    else arr.item()
                )
                for ((k, shape), arr_shape, arr) in zip(
                    self.items(), arr_shapes, arrays
                )
            }
        )

    def flatten2d(self, values):
        assert all(
            ((np.shape(values[k]) == (shape * 2)) for (k, shape) in self.items())
        )
        return block_diag(
...
def prod(iterable, *arg: Tuple[_M]):
...
def add_arrays(*arrays: np.ndarray):
...
def aggregate(array, axis=None, **kwargs):
    """
