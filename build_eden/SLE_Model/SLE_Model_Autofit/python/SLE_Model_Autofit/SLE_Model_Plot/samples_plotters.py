...
class SamplesPlotter:
    def __init__(self, samples, output=Output()):
    ...
    def log_plot_exception(self, plot_name):
        """
        ...
        """
        logger.info(
            f"{self.__class__.__name__} unable to produce {plot_name} visual: posterior estimate thereforenot yet sufficient for this model-fit is not yet robust enough to do this. Visualshould be produced in later update, once posterior estimate is updated."
        )
...
class MCMCPlotter(SamplesPlotter):
    def _plot_trajectories(self, samples, log_posterior_list, **kwargs):
        ...
        for i in range(self.samples.model.prior_count):
            for walker_index in range(log_posterior_list.shape[1]):
                ax = axes[i]
                ax.plot(
                    samples[:, walker_index, i],
                    log_posterior_list[:, walker_index],
                    alpha=0.3,
                )
    ...
    def _plot_time_series(self, samples, **kwargs):
        (fig, axes) = plt.subplots(
            self.samples.model.prior_count, figsize=(10, 7), sharex=True
        )
        for i in range(self.samples.model.prior_count):
            ax = axes[i]
