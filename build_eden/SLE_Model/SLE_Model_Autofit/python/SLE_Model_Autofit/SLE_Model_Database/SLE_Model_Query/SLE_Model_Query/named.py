...
from SLE_Model_Autofit.SLE_Model_Database.SLE_Model_Query import condition as c
from SLE_Model_Autofit.SLE_Model_Database.SLE_Model_Query.junction import (
    AbstractJunction,
)
from SLE_Model_Autofit.SLE_Model_Database.SLE_Model_Query.SLE_Model_Query.abstract import (
    AbstractQuery,
)
...
def _make_comparison(symbol, other):
    ...
    if other is None:
        if symbol != "=":
            raise AssertionError("Inequalities to None do not make sense")
        return c.NoneCondition()
    if isinstance(other, str):
        return c.StringValueCondition(symbol, other)
    if isinstance(other, Real):
        return c.ValueCondition(symbol, other)
    if inspect.isclass(other):
        if symbol != "=":
            raise AssertionError("Inequalities to types do not make sense")
        return c.TypeCondition(other)
    raise AssertionError(f"Cannot evaluate equality to type {type(other)}")


class NamedQuery(AbstractQuery):
    def __init__(self, name, condition=None, inverted=False):
        """
    ...
    def __invert__(self):
        return NamedQuery(
            name=self.name, condition=self._condition, inverted=(not self._inverted)
        )
    ...
    def condition(self):
        ...
        """
        condition = c.NameCondition(self.name)
        if super().condition:
    ...
    def __call__(self, *args, **kwargs):
        raise AttributeError(f"'Aggregator' object has no attribute '{self.name}'")
    ...
    def tables_string(self):
        ...
        if len(tables) > 2:
            raise AssertionError("Currently maximum of 2 tables supported")
        return string
    ...
    def __getattr__(self, item):
        ...
        if self.other_condition is None:
            return NamedQuery(self.name, NamedQuery(item))
        if isinstance(self.other_condition, NamedQuery):
            return NamedQuery(self.name, getattr(self.other_condition, item))
        raise AttributeError("Can only extend a simple path query")
    ...
    def __eq__(self, other):
        ...
        return self._recursive_comparison("=", other)
    ...
    def _recursive_comparison(self, symbol, other):
        ...
        """
        if self.other_condition is None:
            return NamedQuery(self.name, _make_comparison(symbol, other))
        if isinstance(self.other_condition, NamedQuery):
            return NamedQuery(
                self.name,
                cast(NamedQuery, self.other_condition)._recursive_comparison(
                    symbol, other
                ),
            )
        if isinstance(self._condition, AbstractJunction):
            raise AssertionError("Cannot compare a complex query")
        raise AssertionError(f"Cannot evaluate equality to type {type(other)}")
