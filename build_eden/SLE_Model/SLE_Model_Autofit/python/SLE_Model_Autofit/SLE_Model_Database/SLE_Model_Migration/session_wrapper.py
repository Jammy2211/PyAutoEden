...
class SessionWrapper:
    ...
    def _init_revision_table(self):
        ...
        """
        self.session.execute("CREATE TABLE revision (revision_id VARCHAR PRIMARY KEY)")
        self.session.execute("INSERT INTO revision (revision_id) VALUES (null)")
    ...
    def is_table(self):
        ...
        """
        try:
            self.session.execute("SELECT 1 FROM revision")
            return True
    ...
    def revision_id(self):
        ...
        """
        for row in self.session.execute("SELECT revision_id FROM revision"):
            return row[0]
        return None
    ...
    def revision_id(self, revision_id):
        self.session.execute(
            sa.text(f"UPDATE revision SET revision_id = :revision_id"),
