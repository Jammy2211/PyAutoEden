...
import numpy as np
from SLE_Model_Autoconf import conf, cached_property
from SLE_Model_Autofit import exc
from SLE_Model_Autofit.SLE_Model_Database.sqlalchemy_ import sa
from SLE_Model_Autofit.SLE_Model_Graphical import (
    MeanField,
    AnalysisFactor,
    _HierarchicalFactor,
    FactorApproximation,
...
from SLE_Model_Autofit.SLE_Model_NonLinear.SLE_Model_Paths.directory import (
    DirectoryPaths,
)
...
from SLE_Model_Autofit.SLE_Model_NonLinear.timer import Timer
...
from SLE_Model_Autofit.SLE_Model_NonLinear.SLE_Model_Paths.null import NullPaths
...
class NonLinearSearch(AbstractFactorOptimiser, ABC):
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__()
        from SLE_Model_Autofit.SLE_Model_NonLinear.SLE_Model_Paths.database import (
            DatabasePaths,
        )
    ...
    def optimise(self, factor_approx, status=Status()):
        ...
        if not isinstance(factor, (AnalysisFactor, PriorFactor, _HierarchicalFactor)):
            raise NotImplementedError(
                f"Optimizer {self.__class__.__name__} can only be applied to AnalysisFactors, HierarchicalFactors and PriorFactors"
            )
    ...
    def fit(
        ...
        analysis.visualize_before_fit(paths=self.paths, model=model)
    ...
    def config_dict_run(self):
        ...
        if os.environ.get("PYAUTOFIT_TEST_MODE") == "1":
            logger.warning(
                f"""TEST MODE ON: SEARCH WILL SKIP SAMPLING
...
"""
    ...
    def make_sneaky_pool(self, fitness_function):
        ...
        """
        self.logger.warning(
            "...using SneakyPool. This copies the likelihood function to each process on instantiation to avoid copying multiple times."
        )
        return SneakyPool(
