...
class SamplesPDF(Samples):
    ...
    def from_table(cls, filename, model, number_live_points=None):
        ...
        """
        from SLE_Model_Autofit.SLE_Model_NonLinear.SLE_Model_Samples.stored import (
            SamplesStored,
        )
    ...
    def values_at_upper_sigma(self, sigma, as_instance=True):
        ...
        return list(
            map(
                (lambda param: param[1]), self.values_at_sigma(sigma, as_instance=False)
            )
        )
    ...
    def values_at_lower_sigma(self, sigma, as_instance=True):
        ...
        return list(
            map(
                (lambda param: param[0]), self.values_at_sigma(sigma, as_instance=False)
            )
        )
