...
if TYPE_CHECKING:
    ...
    from SLE_Model_Autofit.SLE_Model_Mapper.variable_operator import (
...
class Plate:
    ...
    def make_index_seq(self, plates_index, plate_sizes):
...
class Variable(ModelObject):
    def __init__(self, name=None, *plates: Plate, id_=None):
        """
    ...
    def make_indexes(self, plates_index, plate_sizes):
...
def broadcast_plates(value, in_plates, out_plates, reducer=np.sum):
    """
    ...
    moved_value = np.moveaxis(
        np.expand_dims(value, tuple(range((n_in + shift), k))), in_axes, out_axes
    )
