...
from SLE_Model_Autogalaxy.SLE_Model_Profiles.geometry_profiles import GeometryProfile
        ...
        for (name, val) in kwargs.items():
            if isinstance(val, list):
                raise exc.GalaxyException(
                    "One or more of the input light / mass profiles has been passed to the Galaxy objectas a list.The Galaxy object cannot accept a list of light / mass profiles. Instead, pass these objects as a dictionary, where the key of each dictionary entry isthe name of the profile and the value is the profile, e.g.:{bulge : al.lp.Sersic()}"
                )
...
{}""".format(
                """
""".join(
                    map(str, self.cls_list_from(cls=LightProfile))
                )
            )
...
{}""".format(
                """
""".join(
                    map(str, self.cls_list_from(cls=MassProfile))
                )
            )
...
{}""".format(
    ...
    def _radial_projected_shape_slim_from(self, grid):
        """
