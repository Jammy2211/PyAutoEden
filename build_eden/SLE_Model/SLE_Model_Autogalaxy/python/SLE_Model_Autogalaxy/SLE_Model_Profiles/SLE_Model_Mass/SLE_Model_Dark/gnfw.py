...
from SLE_Model_Autogalaxy.SLE_Model_Profiles.SLE_Model_Mass.SLE_Model_Dark.abstract import (
    AbstractgNFW,
)
...
class gNFW(AbstractgNFW):
    ...
    def convergence_func(self, grid_radius):
        ...
        for index in range(grid_radius.shape[0]):
            integral_y_value = quad(
                integral_y, a=0.0, b=1.0, args=grid_radius[index], epsrel=gNFW.epsrel
            )[0]
            grid_radius[index] = (
    ...
    def potential_func(
        ...
        angle = potential_integral[i] + (
            ((potential_integral[(i + 1)] - potential_integral[i]) * (_eta_u - r1))
            / (r2 - r1)
        )
