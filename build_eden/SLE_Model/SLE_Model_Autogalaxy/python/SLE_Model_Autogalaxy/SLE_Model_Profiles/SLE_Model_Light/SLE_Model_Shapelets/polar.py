...
from SLE_Model_Autogalaxy.SLE_Model_Profiles.SLE_Model_Light.SLE_Model_Shapelets.abstract import (
    AbstractShapelet,
)


class ShapeletPolarEll(AbstractShapelet):
    def __init__(self, n, m, centre=(0.0, 0.0), ell_comps=(0.0, 0.0), beta=1.0):
        """
    ...
    def image_2d_from(self, grid, operated_only=None):
        ...
        laguerre = genlaguerre(
            n=((self.n - np.abs(self.m)) / 2.0), alpha=np.abs(self.m)
        )
        ...
        return np.abs(
            (
                (((const * (radial ** np.abs((self.m / 2.0)))) * shapelet) * gauss)
...
class ShapeletPolar(ShapeletPolarEll):
    def __init__(self, n, m, centre=(0.0, 0.0), beta=1.0):
        """
        Shapelets where the basis function is defined according to a Polar (r,theta) grid of coordinates.
