...
class ShapeletCartesianEll(AbstractShapelet):
    def __init__(self, n_y, n_x, centre=(0.0, 0.0), ell_comps=(0.0, 0.0), beta=1.0):
        """
    ...
    def image_2d_from(self, grid, operated_only=None):
        ...
        return (
            (
                (shapelet_y * shapelet_x)
                * np.exp((((-0.5) * ((y**2) + (x**2))) / (self.beta**2)))
            )
            / self.beta
        ) / np.sqrt(
            (
                (((2 ** (self.n_x + self.n_y)) * np.pi) * factorial(self.n_y))
                * factorial(self.n_x)
            )
        )
...
class ShapeletCartesian(ShapeletCartesianEll):
    def __init__(self, n_y, n_x, centre=(0.0, 0.0), beta=1.0):
        """
        Shapelets where the basis function is defined according to a Cartesian (y,x) grid of coordinates.
