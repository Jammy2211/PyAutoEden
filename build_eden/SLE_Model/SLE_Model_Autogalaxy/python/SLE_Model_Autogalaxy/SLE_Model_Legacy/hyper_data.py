class HyperImageSky:
    ...
    def hyper_image_from(self, image):
        """
        Returns a new with the background sky level hyper. This can simply multiple by a instance factor         (assuming a uniform background sky) because the is in units electrons per second.
...
class HyperBackgroundNoise:
    ...
    def hyper_noise_map_complex_from(self, noise_map):
        """
        Returns a hyper noise_maps from the background noise_maps grid.
        ...
        ----------
            ...
            background_noise
                The background noise_maps values (electrons per second).
        """
        return noise_map + ((1.0 + 1j) * self.noise_scale)
