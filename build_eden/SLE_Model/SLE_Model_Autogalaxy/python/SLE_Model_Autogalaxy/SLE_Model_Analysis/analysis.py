...
from SLE_Model_Autogalaxy.SLE_Model_Analysis import model_util
...
class Analysis(af.Analysis):
    def __init__(self, cosmology=Planck15):
        """
...
class AnalysisDataset(Analysis):
    ...
    def set_preloads(self, paths, model):
        """
    ...
    def hyper_image_sky_via_instance_from(self, instance):
        """
    ...
    def hyper_background_noise_via_instance_from(self, instance):
        """
    ...
    def check_and_replace_adapt_images(self, paths):
        """
        ...
        try:
            ...
            if np.max(abs((adapt_model_image - self.adapt_model_image))) > 1e-08:
                logger.info(
                    "ANALYSIS - Hyper image loaded from pickle different to that set in Analysis class.Overwriting hyper images with values loaded from pickles."
                )
                self.adapt_model_image = adapt_model_image
    ...
    def output_or_check_figure_of_merit_sanity(self, paths, result):
        """
            ...
            if conf.instance["general"]["test"]["check_figure_of_merit_sanity"]:
                if not np.isclose(figure_of_merit, figure_of_merit_sanity):
                    raise exc.AnalysisException(
                        f"""Figure of merit sanity check failed. This means that the existing results of a model fit used a different likelihood function compared to the one implemented now.

Old Figure of Merit = {figure_of_merit_sanity}
New Figure of Merit = {figure_of_merit}"""
                    )
