...
class Preloads(ag.Preloads):
    ...
    @classmethod
    def setup_all_via_fits(cls, fit_0, fit_1):
        """
            ...
            if (
                ...
                if (
                    np.max(
                        abs(
                            (
                                traced_grids_of_planes_0[(-1)]
                                - traced_grids_of_planes_1[(-1)]
                            )
                        )
                    )
                    < 1e-08
...
"""
        ...
        line += [
            f"""Use W Tilde = {self.use_w_tilde}

"""
        ]
        line += [
            f"""Blurred Image = {(np.count_nonzero(self.blurred_image) != 0)}
"""
        ]
        line += [
