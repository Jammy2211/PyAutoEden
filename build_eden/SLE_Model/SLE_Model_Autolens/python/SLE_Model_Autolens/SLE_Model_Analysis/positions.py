...
class AbstractPositionsLH:
    def __init__(self, positions, threshold):
        """
        ...
        if len(positions) == 1:
            raise exc.PositionsException(
                f"""The positions input into the Positions have length one (e.g. it is only one (y,x) coordinate and therefore cannot be compared with other images).
...
Please input more positions into the Positions."""
    ...
    def log_likelihood_function_positions_overwrite(self, instance, analysis):
        raise NotImplementedError

    def output_positions_info(self, output_path, tracer):
        """
...
"""
            ...
            f.write(
                f"""Radial Distance from (0.0, 0.0): 
 {distances} 
...
class PositionsLHPenalty(AbstractPositionsLH):
    def __init__(self, positions, threshold, log_likelihood_penalty_factor=100000000.0):
        """
        The `PositionsLH` objects add a penalty term to the likelihood of the **PyAutoLens** `log_likelihood_function`
        defined in the `Analysis` classes.
    ...
    def log_likelihood_penalty_base_from(self, dataset):
        """
