...
from SLE_Model_Autogalaxy.SLE_Model_Analysis.analysis import (
    AnalysisDataset as AgAnalysisDataset,
)
from SLE_Model_Autolens.SLE_Model_Analysis.result import ResultDataset
from SLE_Model_Autolens.SLE_Model_Analysis.maker import FitMaker
...
from SLE_Model_Autolens.SLE_Model_Lens import ray_tracing_util
...
class AnalysisLensing:
    ...
    def tracer_via_instance_from(
        self, instance, profiling_dict=None, tracer_cls=Tracer
    ):
        """
    ...
    def log_likelihood_positions_overwrite_from(self, instance):
        """
    ...
    def raise_exceptions(self, model):
        if ag.util.model.has_pixelization_from(model=model):
            if (
                ...
                and (
                    not conf.instance["general"]["test"][
...
It is likely your model-fit will infer an inaccurate solution.
 ...
 Please read the following readthedocs page for a description of why this is, and how to set upa positions likelihood object:
...
https://pyautolens.readthedocs.io/en/latest/general/demagnified_solutions.html"""
    ...
    def log_likelihood_cap_from(self, stochastic_log_likelihoods_json_file):
        """
        ...
        try:
            ...
            raise exc.AnalysisException(
                "The file 'stochastic_log_likelihoods.json' could not be found in the output of the model-fitting resultsin the analysis before the stochastic analysis. Rerun PyAutoLens with `stochastic_outputs=True` in the`general.ini` configuration file."
            )
    ...
    def save_stochastic_outputs(self, paths, samples):
        """
        Certain `Inversion`'s have stochasticity in their log likelihood estimate (e.g. due to how different KMeans
        seeds change the pixelization constructed by a `VoronoiBrightnessImage` pixelization).
