    ...
    for i in range(grid.shape[0]):
        for j in range(grid.shape[0]):
            separations[(i, j)] = np.sqrt(
                (
                    np.square((grid[(i, 0)] - grid[(j, 0)]))
                    + np.square((grid[(i, 1)] - grid[(j, 1)]))
                )
            )
    ...
    for y in range(edge_start, edge_end):
        for x in range(edge_start, edge_end):
            grid_slim[(grid_index, 0)] = (
                (coordinate[0] - (y * pixel_scales_upscaled[0])) - y_upscale_half
            ) + y_odd_pixel_scale
            grid_slim[(grid_index, 1)] = (
                (coordinate[1] + (x * pixel_scales_upscaled[1])) + x_upscale_half
            ) - x_odd_pixel_scale
            grid_index += 1
    return grid_slim
    ...
    for y in range(shape_of_edge):
        for x in range(shape_of_edge):
            if (
                (y > 0)
                and (x > 0)
                and (y < (shape_of_edge - 1))
                and (x < (shape_of_edge - 1))
            ):
