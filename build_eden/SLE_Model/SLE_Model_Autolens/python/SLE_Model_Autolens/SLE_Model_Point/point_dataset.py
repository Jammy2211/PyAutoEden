...
class PointDataset:
    def __init__(
        self, name, positions, positions_noise_map, fluxes=None, fluxes_noise_map=None
    ):
        """
    ...
    def dict(self):
        ...
        return {
            ...
            "fluxes": (
                list(np.round(self.fluxes, 4)) if (self.fluxes is not None) else None
            ),
            "fluxes_noise_map": (
                list(self.fluxes_noise_map)
    ...
    def from_dict(cls, dict_):
        ...
        return cls(
            ...
            fluxes=(
                aa.ArrayIrregular(dict_["fluxes"])
                if (dict_["fluxes"] is not None)
                else None
            ),
            fluxes_noise_map=(
                aa.ArrayIrregular(dict_["fluxes_noise_map"])
...
class PointDict(dict):
    def __init__(self, point_dataset_list):
        """
