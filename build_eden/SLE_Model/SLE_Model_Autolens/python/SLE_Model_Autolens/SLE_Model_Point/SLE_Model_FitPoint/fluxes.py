...
class FitFluxes(aa.FitDataset):
    def __init__(self, name, fluxes, noise_map, positions, tracer, point_profile=None):
        ...
        if self.point_profile is None:
            ...
            raise exc.PointExtractionException(
                f"For the point-source named {name} the extracted point source was the class {self.point_profile.__class__.__name__} and therefore does not contain a flux component."
            )
