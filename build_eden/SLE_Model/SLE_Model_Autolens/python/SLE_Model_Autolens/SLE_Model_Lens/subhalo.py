...
from SLE_Model_Autolens.SLE_Model_Aggregator.fit_imaging import _fit_imaging_from
...
class SubhaloResult:
    ...
    def _subhalo_array_from(self, values_native):
    ...
    @property
    def stochastic_log_evidences_native(self):
        ...
        return self.grid_search_result._list_to_native(
            [
                (
                    None
                    if (instance is None)
                    else instance.galaxies.subhalo.mass.mass_at_200
                )
        ...
        centres_native[:, :, 0] = self.grid_search_result._list_to_native(
            lst=[
                (
                    None
                    if (instance is None)
                    else instance.galaxies.subhalo.mass.centre[0]
                )
                for instance in instance_list
            ]
        )
        centres_native[:, :, 1] = self.grid_search_result._list_to_native(
            lst=[
                (
                    None
                    if (instance is None)
                    else instance.galaxies.subhalo.mass.centre[1]
                )
                for instance in instance_list
            ]
        )
...
class SubhaloPlotter(AbstractPlotter):
    ...
    def detection_array_from(self, remove_zeros=False):
    ...
    def figure_with_detection_overlay(
        ...
        fit_imaging_plotter.figures_2d_of_planes(
            plane_index=(-1), subtracted_image=True
        )

    def figure_with_mass_overlay(self, image=False, transpose_array=False):
    ...
    def subplot_detection_imaging(self, remove_zeros=False):
