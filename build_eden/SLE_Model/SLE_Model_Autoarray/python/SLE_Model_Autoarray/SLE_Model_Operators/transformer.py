...
from SLE_Model_Autoarray.SLE_Model_Operators import transformer_util
...
class TransformerNUFFT(NUFFT_cpu, PyLopsOperator):
    def __init__(self, uv_wavelengths, real_space_mask):
        ...
        self.shift = np.exp(
            (
                ...
                * (
                    (
                        ((self.grid.pixel_scales[0] / 2.0) * units.arcsec.to(units.rad))
        ...
        self.real_space_pixels = self.real_space_mask.pixels_in_mask
        self.total_visibilities = int(
            (uv_wavelengths.shape[0] * uv_wavelengths.shape[1])
        )
        self.shape = (
            int(np.prod(self.total_visibilities)),
            int(np.prod(self.real_space_pixels)),
    ...
    def initialize_plan(self, ratio=2, interp_kernel=(6, 6)):
        ...
        self.plan(
            ...
            Kd=(
                (ratio * self.grid.shape_native[0]),
                (ratio * self.grid.shape_native[1]),
            ),
            Jd=interp_kernel,
        )
    ...
    def transform_mapping_matrix(self, mapping_matrix):
        transformed_mapping_matrix = 0 + (
            0j * np.zeros((self.uv_wavelengths.shape[0], mapping_matrix.shape[1]))
        )
    ...
    def adjoint_lop(self, y):
        ...
        y = a_complex_from(
            a_real=y[: int((self.shape[0] / 2.0))],
            a_imag=y[int((self.shape[0] / 2.0)) :],
        )
