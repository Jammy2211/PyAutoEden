        ...
        for i in range(profile_call_max):
            ...
            if i == 5:
                raise exc.ProfilingException(
                    f"Attempt to make profiling dict failed, because a function has beencalled more than {profile_call_max} times, exceed the number of timesa profiled function may be called"
                )
        return result
