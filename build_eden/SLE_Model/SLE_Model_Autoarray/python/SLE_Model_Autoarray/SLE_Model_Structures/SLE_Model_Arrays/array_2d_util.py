...
def check_array_2d_and_mask_2d(array_2d, mask_2d):
    """
...
def convert_array_2d(array_2d, mask_2d, store_native=False, skip_mask=False):
    """
    The `manual` classmethods in the `Array2D` object take as input a list or ndarray which is returned as an
    Array2D.
...
def convert_array_2d_to_slim(array_2d, mask_2d):
    """
    The `manual` classmethods in the `Array2D` object take as input a list or ndarray which is returned as an
    Array2D.
...
@numba_util.jit()
def resized_array_2d_from(array_2d, resized_shape, origin=((-1), (-1)), pad_value=0.0):
    """
    ...
    for (y_resized, y) in enumerate(range(y_min, y_max)):
        for (x_resized, x) in enumerate(range(x_min, x_max)):
            if (
                (y >= 0)
                and (y < array_2d.shape[0])
                and (x >= 0)
                and (x < array_2d.shape[1])
            ):
                if (
                    (y_resized >= 0)
...
def replace_noise_map_2d_values_where_image_2d_values_are_negative(
    ...
    for y in range(image_2d.shape[0]):
        for x in range(image_2d.shape[1]):
            if image_2d[(y, x)] < 0.0:
                absolute_signal_to_noise = (
                    np.abs(image_2d[(y, x)]) / noise_map_2d[(y, x)]
                )
                if absolute_signal_to_noise >= target_signal_to_noise:
                    noise_map_2d[(y, x)] = (
                        np.abs(image_2d[(y, x)]) / target_signal_to_noise
                    )
    return noise_map_2d


@numba_util.jit()
def index_2d_for_index_slim_from(indexes_slim, shape_native):
    """
...
@numba_util.jit()
def index_slim_for_index_2d_from(indexes_2d, shape_native):
    """
...
def numpy_array_2d_via_fits_from(file_path, hdu, do_not_scale_image_data=False):
    """
...
def header_obj_from(file_path, hdu):
    """
    Read a 2D NumPy array from a .fits file.
