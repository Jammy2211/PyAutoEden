...
if TYPE_CHECKING:
    from SLE_Model_Autoarray.SLE_Model_Structures.SLE_Model_Arrays.uniform_1d import (
        Array1D,
    )
...
from SLE_Model_Autoarray.SLE_Model_Structures.SLE_Model_Grids import grid_1d_util
...
class Grid1D(Structure):
    def __init__(self, values, mask, store_native=False):
        """
    ...
    @classmethod
    def no_mask(cls, values, pixel_scales, origin=(0.0,)):
        """
    ...
    def from_mask(cls, mask):
        ...
        grid_1d = grid_1d_util.grid_1d_slim_via_mask_from(
            mask_1d=np.array(mask), pixel_scales=mask.pixel_scales, origin=mask.origin
        )
        return Grid1D(values=grid_1d, mask=mask)

    @classmethod
    def uniform(cls, shape_native, pixel_scales, origin=(0.0, 0.0)):
        """
        ...
        grid_slim = grid_1d_util.grid_1d_slim_via_shape_slim_from(
            shape_slim=shape_native, pixel_scales=pixel_scales, origin=origin
        )
        return cls.no_mask(values=grid_slim, pixel_scales=pixel_scales, origin=origin)

    @classmethod
    def uniform_from_zero(cls, shape_native, pixel_scales):
        """
        Create a `Grid1D` (see `Grid`D.__new__`) as a uniform grid of (x) values given an input `shape_native` and
        ...
        return cls.no_mask(values=grid_slim, pixel_scales=pixel_scales)
    ...
    def grid_2d_radial_projected_from(self, angle=0.0):
        """
