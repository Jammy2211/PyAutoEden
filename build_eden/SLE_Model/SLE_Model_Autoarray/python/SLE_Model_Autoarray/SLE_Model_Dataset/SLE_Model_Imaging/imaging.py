...
class Imaging(AbstractDataset):
    def __init__(
        ...
        if pad_for_convolver and (psf is not None):
            try:
                ...
                logger.info(
                    f"The image and noise map of the `Imaging` objected have been padded to the dimensions{data.shape}. This is because the blurring region around the mask (which defines wherePSF flux may be convolved into the masked region) extended beyond the edge of the image.This can be prevented by using a smaller mask, smaller PSF kernel size or manually paddingthe image and noise-map yourself."
                )
        super().__init__(
            data=data,
            noise_map=noise_map,
    ...
    def output_to_fits(
        self, data_path, psf_path=None, noise_map_path=None, overwrite=False
    ):
