...
def noise_map_via_weight_map_from(weight_map):
    ...
    The weight map may contain zeros, in which case the variances are converted to large values to omit them from     the analysis.
...
def setup_random_seed(seed):
    ...
    """
    if seed == (-1):
        seed = np.random.randint(0, int(1000000000.0))
    np.random.seed(seed)
...
def data_with_complex_gaussian_noise_added(data, sigma, seed=(-1)):
    ...
    return (data + gaussian_noise[:, 0]) + (1j * gaussian_noise[:, 1])
...
def visibilities_noise_map_with_signal_to_noise_limit_from(
    ...
    from SLE_Model_Autoarray.SLE_Model_Structures.visibilities import (
