...
@numba_util.jit()
def zeroth_regularization_matrix_from(coefficient, pixels):
    """
...
@numba_util.jit()
def constant_regularization_matrix_from(coefficient, neighbors, neighbors_sizes):
    """
    ...
    for i in range(parameters):
        regularization_matrix[(i, i)] += 1e-08
...
def constant_zeroth_regularization_matrix_from(
    coefficient, coefficient_zeroth, neighbors, neighbors_sizes
):
    """
    From the pixel-neighbors array, setup the regularization matrix using the instance regularization scheme.
...
def adaptive_regularization_weights_from(
    ...
    """
    return (
        (inner_coefficient * pixel_signals)
        + (outer_coefficient * (1.0 - pixel_signals))
    ) ** 2.0


def brightness_zeroth_regularization_weights_from(coefficient, pixel_signals):
    """
...
def weighted_regularization_matrix_from(
    regularization_weights, neighbors, neighbors_sizes
):
    """
...
@numba_util.jit()
def brightness_zeroth_regularization_matrix_from(regularization_weights):
    """
...
def reg_split_from(splitted_mappings, splitted_sizes, splitted_weights):
    """
...
def pixel_splitted_regularization_matrix_from(
    ...
    regularization_weight = regularization_weights**2.0
    for i in range(parameters):
        regularization_matrix[(i, i)] += 2e-08
        for j in range(4):
            k = (i * 4) + j
