...
from SLE_Model_Autoarray.SLE_Model_Util.fnnls import fnnls_cholesky
...
def curvature_matrix_with_added_to_diag_from(
    curvature_matrix, value, no_regularization_index_list=None
):
    """
...
def reconstruction_positive_negative_from(
    ...
    force_check_reconstruction=False,
):
    """
...
def reconstruction_positive_only_from(
    data_vector, curvature_reg_matrix, settings=SettingsInversion()
):
    """
...
def preconditioner_matrix_via_mapping_matrix_from(
    mapping_matrix, regularization_matrix, preconditioner_noise_normalization
):
    """
    ...
    curvature_matrix = curvature_matrix_via_mapping_matrix_from(
        mapping_matrix=mapping_matrix, noise_map=np.ones(shape=mapping_matrix.shape[0])
    )
    return (
