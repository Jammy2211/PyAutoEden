...
class Preloads:
    ...
    def set_w_tilde_imaging(self, fit_0, fit_1):
        ...
        if not fit_0.inversion.has(cls=AbstractMapper):
            return
        if np.max(abs((fit_0.noise_map - fit_1.noise_map))) < 1e-08:
    ...
    def set_relocated_grid(self, fit_0, fit_1):
        ...
        if fit_0.inversion is None:
            return
        if (fit_0.inversion.total(cls=AbstractMapper) > 1) or (
            fit_0.inversion.total(cls=AbstractMapper) == 0
        ):
            return
    ...
    def set_curvature_matrix(self, fit_0, fit_1):
        ...
        if inversion_0.curvature_matrix.shape == inversion_1.curvature_matrix.shape:
            if (
                np.max(
                    abs((inversion_0.curvature_matrix - inversion_1.curvature_matrix))
                )
                < 1e-08
            ):
    ...
    def check_via_fit(self, fit):
        ...
        try:
            ...
            if data_vector_difference > 0.0001:
                raise exc.PreloadsException(
                    f"""
            ...
            if curvature_reg_matrix_difference > 0.0001:
                raise exc.PreloadsException(
                    f"""
...
"""
        ...
        line += [
            f"""Mapper = {(self.mapper_list is not None)}
"""
        ]
        line += [
...
"""
        ...
        line += [
            f"""Curvature Matrix = {(self.curvature_matrix is not None)}
"""
        ]
        line += [
